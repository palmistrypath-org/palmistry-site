...
<Checkpoint title="Checkpoint — Foundations">
	<CheckpointItem question="Someone holds out a palm for you to read for the first time. What do you observe before you look at a single line, and why does that order matter?">
		The structure first: the hand's overall shape, the skin's texture, how much the hand yields when pressed, and then the thumb and fingers. The order matters because structure is the broadest statement the hand makes, and it becomes the frame every smaller feature is read against. Interpreting a line before you know whether the hand is firm or soft, coarse or fine, means reading a detail with no context to place it in.
	</CheckpointItem>
	<CheckpointItem question="Shape, texture, and flexibility are three separate observations. Give one sentence on each for your own active hand — then say which of the three the elemental shape classification actually depends on.">
		Shape depends on two proportions only: the shape of the palm and the length of the fingers relative to it. Those two inputs alone determine the elemental classification. Texture (how fine or coarse the skin's grain is) and flexibility or consistency (how much the hand yields under pressure) are separate qualifying observations — they refine what the shape suggests, but they never change which shape the hand is.
	</CheckpointItem>
