...
<Checkpoint title="Checkpoint — The Mounts">
	<CheckpointItem question="There are seven planetary mounts but eight mount regions on the palm. Point to the relevant areas on your own hand and explain the discrepancy.">
		Mars occupies two separate positions — Lower Mars, the strip inside the life line between Jupiter and Venus, and Upper Mars, at the percussion edge between Mercury and Luna. The Plain of Mars, the central palm between them, is counted as its own region rather than belonging to either. Seven planets, eight regions: Venus, Jupiter, Saturn, Apollo, Mercury, Luna, and Mars twice, plus the Plain between.
	</CheckpointItem>
	<CheckpointItem question="Assessing a mount takes two observations, not one. What are they, and how do you make the second?">
		How raised the mount is, and how it responds to pressure. You make the second by pressing it. The two are independent: a mount can be visibly prominent but soft and yielding, or barely raised but distinctly firm, and those read differently. A judgement based on appearance alone has done only half the assessment.
	</CheckpointItem>
