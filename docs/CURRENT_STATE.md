...
`src/content/blog/` contains a substantial beginner-focused article library covering major/minor lines and variations, hand/finger/nail topics, traditions, myths, books, worksheets, and related search-oriented topics. Intermediate/advanced blog directories currently exist but the observed populated library is concentrated under beginner content. `content-audit` currently counts 58 blog posts; `editorial-backlog.md`'s "Published" figure tracks only articles from its own Next 10/25/Later roadmap list, not the full file count, so the two numbers are not expected to match. Relay PP-RELAY-020 (2026-08-19) added `beginner/phalanges-palmistry.md`, a dedicated treatment of the three-phalange-per-finger framework, sourced from Benham (1900) and Gettings (1965), with a reciprocal link to/from `beginner/finger-shapes-palmistry.md` (which already carried a brief summary of the same framework as one sub-section). The same task also corrected `editorial-backlog.md`'s stale status for `beginner/forked-head-line-meaning.md`, which was already live on `main` but still listed as an open Next 10 item. Relay PP-RELAY-021 (2026-08-19) added `beginner/is-palmistry-real.md`, an evidence/trust FAQ for skeptical readers, distinct from `beginner/what-palmistry-is.md`'s history-and-definition overview; it separates what historical palmists (Cheiro, Benham, Varāhamihira) actually claimed from what the evidence does and doesn't establish, and adds one reciprocal navigation link from `what-palmistry-is.md`.
