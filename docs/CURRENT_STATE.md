...
Relay PP-RELAY-011 (2026-08-18) replayed the same audited `<Practice>` wrapper treatment across all six current Lines lessons and added the audited end-of-module Checkpoint to the closing lesson, per `docs/audits/CURRICULUM_WAVE_3E_3F_BRANCH_AUDIT_2026-08.md`. `05-fate-line.mdx` now carries a five-item `<Checkpoint title="Checkpoint — The Major Lines">` using the existing `Checkpoint.astro`/`CheckpointItem.astro` components (unmodified, first added by PP-RELAY-010), matching the audited branch's post-correction wording and recapping only material already taught earlier in the module. One unrelated frontmatter `description` wording change bundled into the audited branch's commit for `line-quality-and-markings.mdx` was deliberately not replayed, for the same reason as PP-RELAY-010's `thumb-and-fingers.mdx` exclusion; that description is unchanged from current `main`. The pre-existing life-line testing-claim wording in `04-life-line.mdx` was left untouched at the time (later closed by Relay PP-RELAY-024; see below). Foundations, Lines, and three of Advanced's six lessons now use `<Practice>`; the remaining Batch 3F capstone body revision and Sun/Mercury quote-fidelity rewrite remain deliberately out of scope.

Relay PP-RELAY-012 (2026-08-18) replayed the same audited `<Practice>` wrapper treatment across all eight current Mounts lessons and added the audited end-of-module Checkpoint to the closing lesson, per `docs/audits/CURRICULUM_WAVE_3E_3F_BRANCH_AUDIT_2026-08.md`. `01-mounts-overview.mdx`'s former "What you're not doing yet" heading/paragraph is now a `<Practice title="Try it: survey the landscape">` block, matching the audited branch exactly. `08-mount-of-mars.mdx` now carries a four-item `<Checkpoint title="Checkpoint — The Mounts">` using the existing `Checkpoint.astro`/`CheckpointItem.astro` components (unmodified), recapping only the seven-planetary-mounts/eight-region model and assessment method already taught earlier in the module. One unrelated frontmatter `description` wording change bundled into the audited branch's commit for `08-mount-of-mars.mdx` was deliberately not replayed, for the same reason as PP-RELAY-010/011's exclusions; that description is unchanged from current `main`. Foundations, Lines, Mounts, and three of Advanced's six lessons now use `<Practice>`; the remaining Batch 3F capstone body revision and Sun/Mercury quote-fidelity rewrite remain deliberately out of scope.
...
`src/content/blog/` contains a substantial beginner-focused article library covering major/minor lines and variations, hand/finger/nail topics, traditions, myths, books, worksheets, and related search-oriented topics. Intermediate/advanced blog directories currently exist but the observed populated library is concentrated under beginner content. `content-audit` currently counts 60 blog posts; `editorial-backlog.md`'s "Published" figure tracks only articles from its own Next 10/25/Later roadmap list, not the full file count, so the two numbers are not expected to match. Relay PP-RELAY-020 (2026-08-19) added `beginner/phalanges-palmistry.md`, a dedicated treatment of the three-phalange-per-finger framework, sourced from Benham (1900) and Gettings (1965), with a reciprocal link to/from `beginner/finger-shapes-palmistry.md` (which already carried a brief summary of the same framework as one sub-section). The same task also corrected `editorial-backlog.md`'s stale status for `beginner/forked-head-line-meaning.md`, which was already live on `main` but still listed as an open Next 10 item. Relay PP-RELAY-021 (2026-08-19) added `beginner/is-palmistry-real.md`, an evidence/trust FAQ for skeptical readers, distinct from `beginner/what-palmistry-is.md`'s history-and-definition overview; it separates what historical palmists (Cheiro, Benham, Varāhamihira) actually claimed from what the evidence does and doesn't establish, and adds one reciprocal navigation link from `what-palmistry-is.md`. Relay PP-RELAY-022 (2026-08-19) added `beginner/short-heart-line-meaning.md`, a focused companion to `beginner/heart-line.md` covering how to confirm a heart line is genuinely short (versus faint, broken, or chained) and the bounded traditional reading — focused emotional investment, not coldness or reduced capacity for feeling — with one reciprocal link added to `heart-line.md`'s existing "Explore further" list. Relay PP-RELAY-023 (2026-08-19) added `beginner/fate-line-starting-points.md`, a focused companion to `beginner/fate-line.md` covering the fate line's four established starting points (wrist/base, Mount of Luna, life line, mid-palm) in depth, grounded in `lessons/lines/05-fate-line.mdx` and `beginner/fate-line.md`, with one reciprocal link added to `fate-line.md`'s "The starting point" subsection. Relay PP-RELAY-024 (2026-08-19) closed the life-line empirical-evidence sourcing gap: `beginner/life-line.md`'s source footer named only Lucas et al. (2019) even though its empirical section names Newrick, Affie & Corrall (1990, *Journal of the Royal Society of Medicine* 83:8, 499–501, 100 autopsies) and Wilson & Mather (1974, *JAMA* 229:11, 1421–1422, 51 cadavers) as well; both citations were verified by targeted lookup and added to the footer. Revision 2 (Director review) found that the article's "the most recent peer-reviewed study" label for Lucas et al. (2019) is an unsupported freshness claim in 2026 and removed it, closing instead with "across these three published studies, the record is mixed and does not establish a reliable predictive relationship." The same revision also reworded `life-line.md`'s FAQ answer and `lessons/lines/04-life-line.mdx`'s lesson-takeaway sentence — both previously "the studies that have tested/directly tested it find no reliable relationship" — to "the limited published studies that have tested it show a mixed record that does not establish a reliable relationship," so the wording cannot be misread as claiming all three studies were negative (Newrick found a significant association; Wilson & Mather and Lucas et al. did not). No new claims, studies, or palmistry interpretations were introduced; see `docs/source-verification-log.md` Pass 6 and Pass 7 for the verification detail. Revision 3 (Director review) corrected the two remaining residual testing summaries in `lessons/lines/04-life-line.mdx` that Pass 7 had flagged but left out of scope: the "myth, corrected" section's "find no reliable relationship" sentence now reads "show a mixed record that does not establish a reliable relationship," and the "has not survived testing" sentence about the older dating schemes now reads "the limited published testing on life-line length and lifespan does not support it." Both now match the "mixed record" wording already used elsewhere in the same file and in `life-line.md`. No new claims, studies, or palmistry interpretations were introduced; see `docs/source-verification-log.md` Pass 8 for the verification detail. Revision 4 (Director review) removed a residual inference Pass 8 had left in the same sentence: "the limited published testing on life-line length and lifespan does not support it" implied the studies tested the dating schemes themselves, when they test only line length against lifespan. The sentence now reads "The older writers did exactly that, with formal dating schemes; this lesson does not teach them," separating the historical fact from Palmistry Path's teaching policy without citing the length-vs-lifespan evidence as if it bore on the dating schemes. No new claims, studies, or palmistry interpretations were introduced; see `docs/source-verification-log.md` Pass 9 for the verification detail.
