...
**Relay task:** PP-RELAY-003, revision 1
...
| `docs/ACTIVE_TASK.md`, `AI_HANDOFF.md`, `ARCHITECTURE.md`, `CHANGELOG.md`, `CURRENT_STATE.md`, `DECISIONS.md` | Stale/mixed, doc-only | Both sides changed independently since the merge-base. `git merge-tree` auto-merges all five with no conflict markers (non-overlapping hunks), but the narrative content should be treated as superseded by current `main` where the two tellings diverge, not reintroduced verbatim. The branch's two 2026-08-13 `DECISIONS.md` entries (order-uniqueness gap; practice layer is permanently stateless) are substantive and worth preserving regardless of merge path. |
...
`git merge-tree $(git merge-base main origin/feat/curriculum-wave-3e-3f) main origin/feat/curriculum-wave-3e-3f` (three-arg form, git 2.43) produced zero conflict markers, independently confirmed. Files changed on both sides since the merge-base are limited to five `docs/*.md` files, all auto-mergeable (non-overlapping hunks). No source, component, or lesson file has textual overlap between what `main` and the branch each changed. Additions (`Practice.astro`, `Checkpoint.astro`, `CheckpointItem.astro`, `combining-what-you-see.mdx`, `advanced/simian-line.mdx`, the redirect stub) and the one removal (`lines/06-simian-line.mdx`, matching the rename) carry no conflict risk. **A merge or rebase of this branch onto current `main` would be textually clean.** The residual risk is narrative, not mechanical: the five doc files' content on `main` has moved on substantially since the merge-base (Relay pilot state) and would need editorial reconciliation, not just a mechanical merge, to avoid reading as internally inconsistent.
...
3. **Lesson `order` uniqueness has no automated check** anywhere in `content-audit` or `audit:all` — confirmed by reproducing the branch's own near-miss. Worth deciding whether to add one; this is a tooling gap, not something this audit's scope authorizes fixing.
4. **Reconciling the five stale `docs/*.md` files** with current `main`'s Relay-pilot narrative is an editorial decision, not a merge-mechanics one.
...
The branch is textually mergeable and validates cleanly, and its content — so far as this audit can verify without the original source editions — stays inside the approved 3E/3F scope with no unrelated drift, no new source tiers, and two self-caught corrections already applied by the branch author. The bounded next step is a **source-verification pass** on the new Cheiro/Benham quotations in `advanced/01-minor-lines-overview.mdx` (item 1 above), followed by a **doc reconciliation pass** to bring the five stale `docs/*.md` files forward onto current `main`'s narrative before any merge — both narrowly scoped, neither requiring new curriculum decisions. Only after those should shipping 3E/3F as a reviewed PR be considered; this audit does not authorize that merge.
