...
## 2026-08-28 - Relay merge-authorization is a shared, tested module plus an additive status check; branch-protection enforcement stays a human action
**Decision:** The merge-authorization invariant (`status: MERGE_APPROVED` with the exact `approved_pr` and full `approved_head_sha`) is implemented once, in `scripts/verify-relay-merge-gate.mjs`, with a fixture-driven self-test (`npm run relay:merge-gate:selftest`, run in CI). `.github/workflows/relay-merge-gate.yml` runs it as a new GitHub check on every `[RELAY ...]` PR to `main`, always reading `.ai-ops/state.json` from the PR's trusted base commit rather than the PR branch. `relay-automerge.yml` and `relay-fastlane.yml` were left untouched — both already implement the same invariant correctly in bash and neither was the actual merge path for the incident below. The worker did not attempt to enable this check as a required status check in the `main` branch protection rule, since no tool available to it can read or modify branch protection/ruleset settings; that remains an explicit human action recorded in `.ai-ops/README.md`.

**Why:** PP-RELAY-057 PR #104 and PP-RELAY-058 PRs #105/#106 merged to `main` while `state.json` still recorded a pre-approval status. `merged_by` on all three is the repository owner's own account, and none of the three carry `relay-automerge.yml`'s "Director-approved squash merge" commit title — proving they were merged directly via GitHub's own merge capability, not through either gated workflow. GitHub allows any account with merge rights to squash-merge an open, green, non-draft PR regardless of what a repository file says; only a required-status-check branch protection rule (or an equivalent merge restriction) can close that specific hole, and that is a repository/account setting outside what files in this repo can control.

**Consequences:** The `relay-merge-gate` check will show red on any Relay PR merged (or still open) without a matching `MERGE_APPROVED` record, but by itself it does not block anything — a human must add it as a required status check for `main` before the incident is structurally prevented rather than merely made visible. Anyone changing the merge-authorization invariant should change `scripts/verify-relay-merge-gate.mjs` (and its fixtures) once, and keep `relay-automerge.yml`'s bash gate consistent with it rather than letting the two drift.
