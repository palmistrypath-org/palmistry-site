...
**Decision:** `.github/workflows/relay-merge-gate.yml`'s `gate-pr` job triggers on `pull_request_target`, not `pull_request`. It still checks out only the PR's base commit (never the PR head) and reads PR number/head SHA/title solely from event metadata passed through `env:`; `gate-refresh` (on `push` to `main` touching `.ai-ops/state.json`) is unchanged. Both jobs publish the same `relay-merge-gate` Statuses API context (`MERGE_GATE_STATUS_CONTEXT` in `scripts/verify-relay-merge-gate.mjs`) for every open `[RELAY ...]` PR, evaluating the unchanged invariant: `status: MERGE_APPROVED` with the exact matching `approved_pr` and `approved_head_sha`. `scripts/verify-relay-merge-gate.mjs`'s self-test now also statically re-parses the checked-in workflow file (`checkWorkflowTrustBoundary`) to confirm `gate-pr` still triggers on `pull_request_target` and its checkout step still pins `base.sha` rather than the PR head. `relay-automerge.yml`, `relay-fastlane.yml`, dispatch, task selection, and risk classes are unchanged.

**Why:** revision 2 (PR #108, closed unmerged) kept `gate-pr` on plain `pull_request`. For a same-repo PR — every Relay worker branch, since this project has no fork PRs — GitHub sources a `pull_request`-triggered run's *workflow definition* from the PR branch itself, not from base alone. A worker PR could therefore rewrite this workflow file or the script it calls, and have that rewritten version execute with `statuses: write` to publish its own passing `relay-merge-gate` status, regardless of what the trusted `state.json` said — checking out `state.json` from the base commit (which revisions 1/2 already did) does not help if the workflow logic evaluating it is itself untrusted. `pull_request_target` closes this because GitHub always sources that trigger's workflow definition from the base branch, independent of the PR under evaluation. Live evidence this was never exploited in production: PR #108's `gate-pr` run instead failed outright on its own head `d11e4ae46d4b8fbfe15062a8544f41221d389c2f` and published no `relay-merge-gate` status at all (`GET /repos/.../commits/{sha}/status` returned `total_count: 0`), so the defect was caught in review, not by a failed required check.

**Consequences:** The revision-2 unresolved external gate is unchanged and still open — no tool available to this Relay worker can read or modify branch-protection settings, so the human owner must still (1) add `relay-merge-gate` as a required status check on `main` and (2) enable "Do not allow bypassing the above settings" (including for administrators), per `.ai-ops/README.md`'s Mechanical auto-merge contract section. Anyone modifying `relay-merge-gate.yml` in the future should keep `gate-pr` on `pull_request_target` with a base-only checkout — the workflow self-test (`npm run relay:merge-gate:selftest`, wired into CI) fails if either regresses.
