...
*Reconciled 2026-09-08 on `astra/visual-golden-slice-reconciliation` for issue #122. Review-only; not merged or deployed to production. Implementation in `src/styles/global.css` is authoritative for exact tokens.*

Palmistry Path is one dark, immersive world: **black, gold, and purple**. Text and imagery emerge from shadow; sections flow into each other; every diagram, card, and product shot sits inside the same darkness. This document records the system as implemented on the review branch. The separately approved product suite from PR #120 is protected and is not regenerated by the site atlas scripts.
...
- `prefers-reduced-motion` disables all of it; without JS everything is visible. The `html.js` class gates the hidden initial state and is added only by the loaded reveal controller. Failed module loading leaves content visible; keyboard focus reveals its containing panel immediately.
...
- **Guide cover:** `public/images/guide/quick-start-cover.webp`. Black cover, gold rules, wireframe hand in a double gold ring on violet nebula. This is retained site promotional artwork. The canonical customer products are the approved PR #120 suite; replacing legacy free-guide delivery belongs to issue #121 after visual approval.
