...
3. Put every image in `<Figure>`; keep captions in HTML, never in the image.
4. Give each opening a contained haze (`.atmosphere` or a page-local `::before`) and use the module's wireframe hand as the faint backdrop where a module context exists.
5. Do not introduce new colours. Tint with `rgba(122,72,201,x)` and `rgba(201,169,110,x)` only.
6. Run `npm run build && npm run audit:all && npm run content-audit` after layout work.
