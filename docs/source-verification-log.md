...
## Legacy Head Line pillar cleanup, revision 10 — `blog/beginner/head-line.md` (Relay PP-RELAY-042, revision 10, 2026-08-23)

Revision 9 (branch `claude/relay-PP-RELAY-042-head-line-source-cleanup-r9`, unmerged) reconstructed the cumulative cleanup of `src/content/blog/beginner/head-line.md` against actual `main` (confirmed identical to revision 9's own base — no intervening change to this file had landed on `main`) and was accepted by the Director as sound in substance. Cumulative Director review found two directly equivalent unsupported popularity comparisons still present in revision 9's writer's-fork wording. Revision 10 was built by taking `main`'s current `head-line.md` (re-confirmed unchanged since revision 9 branched) and re-applying the full sound revision-9 diff, then making the two edits below. No other revision-9 wording was reopened.

**Claim-by-claim disposition of the two residual passages:**

| Passage | Revision 9 wording | Disposition | Revision 10 wording |
|---|---|---|---|
| Writer's-fork section, main article | "Cheiro's own reading of the fork was narrower than the popular version: he described it as giving \"more of what is called a dual mentality and less power of concentration on any one subject\" — mental flexibility, with a cost, not a talent bonus." | Not supported. No repository evidence establishes that an alternative reading of the fork is "popular" — the comparison implies a prevalence/consensus claim about a competing interpretation that is not verified. Per the task's required remediation item 1, removed rather than replaced with an equivalent unsupported synonym (`common`, `usual`, `modern`, `widely known`, `often`, `typically`, `popularly`). Cheiro's verified `dual mentality` quotation is stated directly as Cheiro's own reading, which the source fully supports on its own. | "Cheiro read the fork as giving \"more of what is called a dual mentality and less power of concentration on any one subject\" — mental flexibility, with a cost, not a talent bonus." |
| Myths — "A fork means you're a writer" | "Cheiro's reading of the writer's fork was narrower than the popular idea suggests: mental flexibility, not a talent bonus." | Same unsupported popularity-comparison pattern, found in the required full writer's-fork mention sweep (main section, FAQ, myths). Per remediation item 1, the comparison is removed; Cheiro's reading is stated directly. | "Cheiro read the fork as mental flexibility, with a cost, not a talent bonus." |

**Preserved revision-9 boundary (unchanged, re-verified against the controlling evidence during this revision):** every revision-9 correction not listed above, including the Director-verified Benham direction boundary (`.ai-ops/evidence/PP-RELAY-041-benham-head-line-direction.md`), the writer's-fork label retained only as this article's own descriptive term (not a tradition-wide naming/usage claim), the FAQ's Cheiro quotation (already free of the flagged comparison), the writer's-fork vocation/talent caution as Palmistry Path editorial policy rather than a population claim, and the IQ/cognitive-ability and mental-health cautions as Palmistry Path editorial policy rather than tradition-wide consensus claims.

**No new claim invented; no equivalent unsupported synonym substituted:** neither edit introduces `common`, `usual`, `modern`, `widely known`, `often`, `typically`, `popularly`, or any equivalent prevalence/usage/consensus wording. Both edits state Cheiro's verified reading directly and keep the existing, separately-stated Palmistry Path editorial-policy sentence (that the fork is not treated as proof of writing or creative ability) unchanged and clearly distinguishable from Cheiro's historical interpretation.

**Full writer's-fork mention sweep (main section, FAQ, myths) and full-article prevalence/consensus word search** (`popular`, `common`, `usual`, `modern`, `widely`, `often`, `typically`, `sometimes`, `known as`, `tradition`, `classical`, and equivalents), per the required preflight: no further unsupported naming/usage/popularity/synonym/rationale or competing-popularity claim was found beyond the two corrected above. The remaining `traditionally associated with` framing throughout the article, the named Indian- and Chinese-tradition line names, and the "Common myths" section heading are pre-existing revision-8/9 wording outside this revision's authorized scope and were not reopened.

**Claim-risk heuristic disposition:** `npm run audit:claim-risk -- src/content/blog/beginner/head-line.md` returns 5 findings, identical in substance to the 5 pre-existing findings dispositioned under revision 9 above, none newly introduced and none matching the two categories this revision was authorized to fix: lines 9/11 ("most important") are Cheiro's verified direct quotation/paraphrase, attributed by name; line 45 ("many angles") and line 110 ("many forms") are generic non-population phrasing, not palmistry prevalence/consensus claims; line 49 ("usually") is inside a verified verbatim Cheiro quotation, governed by quotation-fidelity rather than paraphrase rules.

**Verification:** `npm run audit:claim-risk -- src/content/blog/beginner/head-line.md` (5 pre-existing findings, dispositioned above, none newly introduced). `npm run build` (110 pages). `npm run content-audit` (65 blog posts, 25 lessons valid). `npm run audit:all` (links — including the no-orphan check — images, schema, indexability, accessibility all pass). `git diff --check` (clean). Diff is bounded to `src/content/blog/beginner/head-line.md`, this log entry, `docs/CHANGELOG.md`, and the revision-10 result artifact. No other article, the claim-risk tooling, or unrelated legacy Head Line claims were touched.

## Broken Head Line Meaning companion article — `blog/beginner/broken-head-line-meaning.md` (Relay PP-RELAY-043, revision 1, 2026-08-23)

Added item 14 from `docs/editorial-backlog.md`'s Next 25 list. The article was drafted directly against the controlling evidence boundary stated in the task packet (`.ai-ops/tasks/PP-RELAY-043.md`), not against the canonical `lessons/lines/03-head-line.mdx` lesson or the `head-line.md` pillar's existing break prose, which the packet treats as maps to candidate claims rather than proof.

**Controlling evidence used:** William G. Benham, *The Laws of Scientific Hand Reading* (1900), Head Line material. Per the task packet, Benham treats a break as an interruption in the line and, in his system, associates it with interruption/unsteadiness and a lack of concentration/firmness/self-control; he separately discusses repair signs (overlapping ends, sister lines, connecting bars, squares). The packet also documents that Benham's same section contains period medical/psychiatric claims (headaches, illness, insanity, nervous disorder, age/prediction) that are historical source content but are explicitly not authorized as article interpretations for Palmistry Path.

**Claim-by-claim disposition:**

| Claim in the article | Disposition |
|---|---|
| A break is a visible interruption where the line stops and resumes, distinct from a short line (ends, does not resume) and a fork (divides without a gap) | Observational, not sourced to Benham; consistent with the definitions already established site-wide in `short-head-line-meaning.md` and `forked-head-line-meaning.md`. |
| A break is traditionally associated with a change in the continuity/stability of thought/concentration (Benham) | Supported. Matches the task packet's authorized safe article-level interpretation exactly; framed as historical interpretation, not empirical psychology. No mention made of Benham's interruption/unsteadiness/firmness/self-control wording beyond "continuity of thought," per the packet's narrower authorized phrasing. |
| Benham names repair formations — overlapping ends, sister line, connecting bar, square — as ways a break can appear | Supported per the task packet's controlling evidence. The article does not assign a specific individual meaning, degree, or ranking to any one formation, and explicitly states there is no gap-size rule and no ranking between the formations, since the packet does not supply verified meanings at that level of detail. |
| Benham's medical/psychiatric claims (headaches, illness, insanity, nervous disorder, age/prediction) | Not repeated, modernized, implied, or referenced by name anywhere in the article, per the packet's explicit prohibition — including not naming them in order to reject them, which the packet's language ("must not be repeated, modernized, implied, or converted") reads as covering. The "What this article does not read into a break" section states Palmistry Path's own safety boundary directly, without describing what Benham's original text says on this point. |
| No prevalence/consensus claim ("breaks are common/rare", "most readers agree", "all traditions share the interpretation") | None made. The claim-risk scanner initially flagged "most often" (describing where a fork typically divides, a structural/definitional point, not a palmistry-interpretation prevalence claim) and "rare" (inside a sentence explicitly disclaiming any prevalence claim). Both were reworded during drafting to remove the flagged terms entirely rather than relying on context to excuse them — see Verification below. |
| No invented combination with mounts, hand shape, depth, branches, endpoints, the Heart Line, Life Line, or other marks | None made. The "A break in context" section explicitly names these as variables the article does not combine a break with. |
| No fixed measurement/gap-size cutoff or monotonic "wider break = stronger meaning" rule | None introduced. Stated explicitly in both the "Overlaps, sister lines, and other repair formations" section and the FAQ. |
| Reciprocal link from `head-line.md`'s "Breaks" bullet | Added one sentence linking to the new article, consistent with the existing reciprocal-link pattern for `short-head-line-meaning` and `long-head-line-meaning` in the same file's "Length" section. No other wording in `head-line.md`'s pre-existing "Breaks" bullet (including its own "transformation rather than catastrophe" framing) was reopened, since the task scope authorizes only a reciprocal link, not a rewrite of the pillar. |

**Claim-type separation:** direct observation ("Where to look", "Telling a break apart from a short line or a fork") is kept distinct from Benham's named historical interpretation ("What it's traditionally associated with", "Overlaps, sister lines, and other repair formations") and from Palmistry Path's own editorial/safety policy ("What this article does not read into a break").

**Quotation fidelity:** no quotation marks are used anywhere in the article; all Benham content is paraphrased at the level of detail the task packet's controlling evidence supports, since the packet does not supply verbatim Benham wording for this task to quote.

**Verification:** `npm run audit:claim-risk -- src/content/blog/beginner/broken-head-line-meaning.md` initially returned 3 findings (`most`, `often` in one sentence describing fork location; `rare` inside a sentence disclaiming any prevalence claim); all three were removed by rewording rather than argued around, and a second run returns 0 findings. `npm run build` (111 pages). `npm run content-audit` (66 blog posts, 25 lessons valid). `npm run audit:all` (links — including the no-orphan check — images, schema, indexability, accessibility all pass). `git diff --check` (clean). Diff is bounded to the new article file, the `head-line.md` reciprocal link, `docs/editorial-backlog.md`'s published-count/status update, this log entry, `docs/CHANGELOG.md`, and the revision-1 result artifact. No canonical Head Line lesson, claim-risk tooling, or other Head Line variation article was touched.

## Broken Head Line Meaning companion article, revision 2 — `blog/beginner/broken-head-line-meaning.md` (Relay PP-RELAY-043, revision 2, 2026-08-23)

Revision 1 (branch `claude/relay-PP-RELAY-043-broken-head-line-meaning`, PR #75, never merged to `main`) produced a useful, otherwise bounded article, but Director review found residual unsupported prevalence/subjective-generalization wording despite the completed source preflight. Revision 2 was built by taking `main`'s current `head-line.md` (confirmed the "Breaks" bullet targeted by revision 1's reciprocal-link sentence is unchanged since revision 1 branched, notwithstanding the unrelated PP-RELAY-042 revision 10 cleanup elsewhere in that file) and reapplying the full sound revision-1 diff, then making the four edits below. No other revision-1 wording was reopened.

**Claim-by-claim disposition of the four remediated passages:**

| Passage | Revision 1 wording | Disposition | Revision 2 wording |
|---|---|---|---|
| Opening paragraph | "...is one of the more unsettling things beginners notice on their own hand." | Not supported. No repository evidence establishes how beginners as a group react to spotting a break — an unverified subjective population generalization. Per the task's required remediation, removed rather than replaced with an equivalent unsupported synonym (`common`, `usual`, `typical`, `most`, `many`). | "...is easy to jump to conclusions about." — a property of the observation itself (unclear formations invite premature interpretation), not a claim about how beginners as a group feel or react. |
| Break/short-line/fork section, opening sentence | "These three formations get confused with each other constantly, and they are not read the same way." | Not supported. No repository evidence establishes how often these formations are mistaken for one another — an unverified frequency/prevalence claim. Per the task's required remediation, removed rather than replaced with an equivalent unsupported synonym (`often`, `frequently`, `commonly`, `usually`). | "These three formations are easy to mix up, and they are not read the same way." — states a structural property (visual/definitional similarity) rather than a claim about how often confusion actually occurs. |
| "What it's traditionally associated with" section, closing sentence | "This article uses that narrow reading and no other: a break is traditionally associated with a change in the continuity of thought, framed here as a historical interpretive tradition rather than a claim about how the mind actually works." | The restated "traditionally associated with" phrase, standing on its own after the Benham attribution in the preceding sentence, could be read as independently asserting a tradition-wide association rather than staying bound to Benham specifically. Per the task's required remediation to keep the interpretation explicitly bounded to Benham, reworded to name Benham's historical system directly and to explicitly disclaim a tradition-wide view, rather than only implying the boundary through the preceding sentence. | "This article uses only that reading and no other: in Benham's historical system, a break is associated with a change in the continuity of thought, presented here as Benham's historical interpretation rather than a claim about how the mind actually works or a view shared across palmistry traditions as a whole." |
| Gap-size FAQ | "Not in this article. No fixed measurement separates a \"significant\" break from a minor one in the sources this site draws from, and this article does not introduce a rule where a wider gap carries a stronger meaning than a narrower one." | Not accurate. "The sources this site draws from" (plural) implied multiple sources were reviewed for this claim, when Benham is the only controlling evidence used anywhere in the article. Per the task's required remediation, corrected to name Benham directly and singularly. | "Not in this article. Benham — the single source this article draws from — does not set out a fixed measurement separating a \"significant\" break from a minor one, and this article does not introduce a rule where a wider gap carries a stronger meaning than a narrower one." |

**Preserved revision-1 boundary (unchanged, re-verified against the task packet's controlling evidence during this revision):** the break/short-line/fork observational definitions; Benham's association of a break with a change in the continuity/stability of thought, framed as his historical interpretation and not empirical psychology; the repair-formations section (overlapping ends, sister line, connecting bar, square) with no gap-size rule, ranking, or separate fate/timing/outcome attached; the explicit non-combination statement in "A break in context"; the safety-boundary section disclaiming any diagnosis, cognitive-impairment, mental-illness, danger, or predictive-event reading, and disclaiming any prevalence or tradition-wide-consensus claim; the FAQ's remaining three answers; the reciprocal link from `head-line.md`'s "Breaks" bullet; and the "Sources consulted" footer naming Benham as the sole source. No Benham medical/psychiatric claim, fixed measurement/gap-size cutoff, monotonic degree rule, or invented combination reading was introduced by this revision or found remaining from revision 1.

**Full-article re-check for this revision:** re-read the complete article end to end against the source-boundary checklist (prevalence/consensus language, historical/scientific assertions, invented combination readings, vague anonymous authority, quotation fidelity, claim-type separation, safety boundaries, source-count accuracy) beyond the four flagged passages. No additional residual issue was found; the revision-1 disposition above remains accurate for every other passage in the article.

**Claim-risk heuristic disposition:** `npm run audit:claim-risk -- src/content/blog/beginner/broken-head-line-meaning.md` initially returned 1 finding after the revision-2 edits — a heuristic match on "generally" in the reworded "What it's traditionally associated with" sentence, which was disclaiming (not asserting) a view shared across palmistry traditions as a whole. Rather than dispositioning the match as an acceptable false positive, the word was removed by rewording to "as a whole", per the task's preference for rewording over arguing around a flagged term; a second run returns 0 findings.

**Verification:** `npm run audit:claim-risk -- src/content/blog/beginner/broken-head-line-meaning.md` (0 findings after the rewording above). `npm run build` (111 pages). `npm run content-audit` (66 blog posts, 25 lessons valid). `npm run audit:all` (links — including the no-orphan check — images, schema, indexability, accessibility all pass). `git diff --check` (clean). This revision's diff is bounded to the four corrected passages in the article, this log entry, the `docs/CHANGELOG.md` revision-2 entry, the `docs/editorial-backlog.md` shipping-revision attribution, and the revision-2 result artifact; the article file, `head-line.md` reciprocal link, and `docs/editorial-backlog.md` published-count are carried forward unchanged from the sound revision-1 diff since this PR is the first to merge the article to `main`.


## Long Heart Line Meaning companion article — `blog/beginner/long-heart-line-meaning.md` (Relay PP-RELAY-044, revision 1, 2026-08-23)

Added item 15 from `docs/editorial-backlog.md`'s Next 25 list. The article was drafted directly against the controlling evidence boundary stated in the task packet (`.ai-ops/tasks/PP-RELAY-044.md`), not against the canonical `lessons/lines/02-heart-line.mdx` lesson or the `heart-line.md` pillar's existing "broad and sustained emotional engagement" Length prose, which the packet explicitly treats as an unverified map to a candidate claim rather than proof.

**Controlling evidence used:** William G. Benham, *The Laws of Scientific Hand Reading* (1900), Line of Heart chapter. Per the task packet, Benham treats Heart Line length as an interpretive variable and, in his length passage, explicitly associates a Heart Line crossing the entire hand — his own extreme illustrative example — with excessive affective/sentimental emphasis and allowing sentiment to guide decisions. The packet authorizes stating, with explicit attribution, that Benham associated a very extended Heart Line with stronger or more dominant sentimental/affective emphasis, kept historical and qualitative. The packet also documents that Benham's surrounding Heart Line material contains period medical, mortality, event-timing, moralizing, jealousy, and deterministic relationship claims that are not authorized for this article.

**Claim-by-claim disposition:**

| Claim in the article | Disposition |
|---|---|
| A long line is defined observationally as one travelling conspicuously far across the upper palm relative to the hand, compared against the palm's own width rather than a fixed measurement | Observational, not sourced to Benham; consistent with the palm-relative comparative pattern already established site-wide in `long-head-line-meaning.md` and `short-heart-line-meaning.md`, and with the packet's instruction not to define a fixed centimetre/finger-zone/midpoint threshold. |
| Length is distinguished from depth, from endpoint, and from forks/chaining before interpretation | Observational; matches the packet's explicit requirement to differentiate length from endpoint and to avoid collapsing depth/forks/chains into the length reading. |
| Benham associates his "crosses the entire hand" example with a stronger, more dominant sentimental/affective emphasis substantial enough to weigh on decision-making | Supported. Matches the task packet's authorized safe article-level interpretation exactly; framed as Benham's own historical interpretation of one extreme example, not a tradition-wide view, not a graduated degree scale, and not empirical psychology. No verbatim Benham wording was supplied by the packet, so the claim is paraphrased throughout with no quotation marks used for it, matching the PP-RELAY-043 precedent for a packet that supplies a paraphrase rather than verified verbatim text. |
| Benham's surrounding medical, mortality, event-timing, moralizing, jealousy, and deterministic relationship claims | Not repeated, modernized, implied, or referenced by name anywhere in the article, per the packet's explicit prohibition. The "What this article does not read into a long heart line" section states Palmistry Path's own safety boundary directly (no medical, predictive, or relationship-outcome claims; no capacity-to-love, empathy, emotional-maturity, or personality-strength claim), without describing what Benham's original surrounding text says on those points. |
| No invented combination with endpoint, depth, chains, forks, branches, mounts, hand shape, or the Head Line | None made. The "Length in context" section explicitly names these as variables the article does not combine length with, and states Benham's extreme example is not converted into a fixed modern measurement, population average, or cutoff. |
| No fixed measurement/precision cutoff or monotonic "longer = more" degree rule | None introduced. `npm run audit:claim-risk` initially flagged one myth heading disclaiming exactly this rule ("The longer the line, the more loving or emotionally mature the person") for matching the `PRECISION_OR_DEGREE_EXTRAPOLATION` "the longer... the more" pattern; reworded to "Added length on its own means added love or emotional maturity" rather than argued around, per the PP-RELAY-043 precedent for handling a disclaiming sentence that still matches the heuristic. Second run returns 0 findings. |
| Reciprocal link from `heart-line.md`'s "Explore further" list | Added one link to the new article at the front of the existing list, consistent with the existing reciprocal-link pattern for the other heart line companion articles already listed there. No other wording in `heart-line.md` — including its pre-existing Length section's "broad and sustained emotional engagement"-adjacent prose, which the task packet explicitly treats as unverified — was reopened, since the task scope authorizes only a reciprocal link, not a rewrite of the pillar. |

**Claim-type separation:** direct observation ("Where to look", "Confirming the line is actually long") is kept distinct from Benham's named historical interpretation ("What it's traditionally associated with") and from Palmistry Path's own editorial/safety policy ("Length in context", "What this article does not read into a long heart line").

**Quotation fidelity:** no quotation marks are used anywhere in the article for Benham's content; the length claim is paraphrased at the level of detail the task packet's controlling evidence supports, since the packet does not supply verbatim Benham wording for this task to quote. Quotation marks are used only for the article's own myth-heading phrasing (e.g. "A long heart line means you love more, or love better"), which is Palmistry Path's own framing of a misconception, not an attribution to Benham or any named source.

**Verification:** `npm run audit:claim-risk -- src/content/blog/beginner/long-heart-line-meaning.md` initially returned 1 finding (a `PRECISION_OR_DEGREE_EXTRAPOLATION` match on a myth heading disclaiming a graduated "the longer... the more" rule); reworded rather than argued around, and a second run returns 0 findings. `npm run build` (112 pages). `npm run content-audit` (67 blog posts, 25 lessons valid). `npm run audit:all` (links — including the no-orphan check — images, schema, indexability, accessibility all pass). `git diff --check` (clean). Diff is bounded to the new article file, the `heart-line.md` reciprocal link, `docs/editorial-backlog.md`'s published-count/status update, this log entry, `docs/CHANGELOG.md`, and the revision-1 result artifact. No canonical Heart Line lesson, claim-risk tooling, or other Heart Line variation article was touched.

Revision 1 was closed without merge (PR #77) — see the revision 2 entry below for the corrections Director review required before the article could ship.

## Long Heart Line Meaning companion article, revision 2 — `blog/beginner/long-heart-line-meaning.md` (Relay PP-RELAY-044, revision 2, 2026-08-23)

Revision 1 (branch `claude/relay-PP-RELAY-044-long-heart-line-meaning`, PR #77, never merged to `main`) produced an otherwise useful, bounded article, but Director review found three residual source-boundary violations despite the completed source preflight. Revision 2 was built by reapplying the full sound revision-1 diff against current `main` (confirmed unchanged since revision 1 branched) and making the three edits below. No other revision-1 wording was reopened.

**Claim-by-claim disposition of the three remediated passages (each appears twice — once in the identification section, once in the matching FAQ answer):**

| Passage | Revision 1 wording | Disposition | Revision 2 wording |
|---|---|---|---|
| Identification ("Confirming the line is actually long") and length FAQ | "...whether it travels conspicuously far across the upper palm, well past the territory below the index finger, rather than stopping in the hand's middle portion." / "...a line travelling conspicuously far across the upper palm, well past the territory below the index finger, is treated as long." | Not authorized. The task packet explicitly prohibits a fixed finger-zone/midpoint cutoff; "past the territory below the index finger" defines "long" against a specific finger-zone boundary rather than staying purely comparative to the hand being observed. Per the task's required remediation, removed and replaced with comparative-only phrasing; Benham's "crosses the entire hand" example remains as a named historical extreme elsewhere in the article, not a modern threshold. | "...the line travels conspicuously far across the upper palm relative to the hand being observed, rather than stopping partway across it." / "...a line travelling conspicuously far across the upper palm relative to the hand being observed is treated as long." |
| "What it's traditionally associated with" section, closing sentence | "...not as a claim about how anyone's emotional life actually works, a graduated scale where more length simply means more of something, or a view shared across palmistry traditions as a whole." | The "or a view shared across palmistry traditions as a whole" clause is itself an unsupported tradition-wide negative-consensus claim — no repository evidence establishes whether Benham's reading is or is not shared by the wider tradition. Per the task's required remediation, replaced with evidence-bounded scope language that limits the article's claim to Benham specifically without asserting anything, positive or negative, about the broader tradition. | "...not as a claim about how anyone's emotional life actually works or a graduated scale where more length simply means more of something. This article establishes only Benham's reading; it does not claim that reading is, or is not, shared more broadly across the wider palmistry tradition." |
| Endpoint-separation bullet and endpoint FAQ | "A line can be long and still end in any of those positions." / "A heart line can be long and still end in any of the endpoint positions covered in the main heart line guide." | Not authorized. Asserting that a long line can end in *every* listed endpoint position is an unsupported categorical claim beyond what "length and endpoint are separate observations" establishes — no repository evidence confirms every endpoint is compatible with a long line. Per the task's required remediation, reworded to state only that the two are separate, independently-noted observations, without the categorical extension. | "Note each observation on its own terms rather than letting one imply the other." / "The main heart line guide covers endpoint positions separately; this article does not use endpoint position to define what counts as long, and it does not use length to predict where a line will end." |

**Preserved revision-1 boundary (unchanged, re-verified against the task packet's controlling evidence during this revision):** the "Where to look" observational description; the depth-vs-length and fork/chain-vs-length distinctions; Benham's association of his extreme "crosses the entire hand" example with a stronger, more dominant sentimental/affective emphasis, framed as his historical interpretation and not empirical psychology; the "Length in context" non-combination statement; the "What this article does not read into a long heart line" safety-boundary section (no capacity-to-love, relationship-success, empathy, emotional-maturity, medical, predictive, or personality-strength claim, and no prevalence/frequency claim); the myths section, including the reworded degree-extrapolation myth heading from revision 1; the remaining FAQ answers; the reciprocal link from `heart-line.md`'s "Explore further" list; and the "Sources consulted" footer naming Benham as the sole source. No Benham medical/mortality/moralizing/jealousy/relationship-deterministic claim, fixed measurement/finger-zone cutoff, monotonic degree rule, unsupported tradition-wide claim (positive or negative), or invented combination reading was introduced by this revision or found remaining from revision 1.

**Full-article re-check for this revision:** re-read the complete article end to end against the source-boundary checklist (prevalence/consensus language, historical/scientific assertions, invented combination readings, vague anonymous authority, quotation fidelity, claim-type separation, safety boundaries, finger-zone/precision cutoffs, negative-consensus wording, endpoint-vs-length categorical overreach) beyond the three flagged passages. No additional residual issue was found; the revision-1 disposition above remains accurate for every other passage in the article.
...
## Pass 15 — Broken Fate Line Meaning companion article, revision 3: break-definition/gap consistency and a historical no-timing overstatement — `blog/beginner/broken-fate-line-meaning.md` (Relay PP-RELAY-048, revision 3, 2026-08-25)

Revision 2 (PR #86, closed without merge) built cleanly on the Director-verified Cheiro evidence boundary in `.ai-ops/evidence/PP-RELAY-048-cheiro-fate-line-break.md` and passed CI plus independent Director validation, but Director review found two narrow correctness issues in the prose itself. Revision 3 keeps revision 2's structure, evidence map, and bookkeeping approach, and drafts fresh directly against the controlling evidence note, correcting only these two issues and directly equivalent wording.

### Revision-3 corrections

1. **Break-definition/gap consistency.** Revision 2's opening definition ("There's a visible line, then a gap, then — sometimes — more line") and its "What a break actually looks like" section ("A true break is a clean stop... a visible span of plain palm before any line resumes") stated that every true break requires a plain gap before resumption. That contradicted the same section's own overlapping-break description, where the new segment begins *before* the earlier one ends and there is no gap. Revision 3 redefines a break as a discontinuity between separate line segments, without asserting a universal gap requirement, and then consistently distinguishes: a **clean break** has a gap along the line's course; Cheiro's specifically verified **overlapping break** has a separate new segment beginning before the earlier segment ends. The two-arrangement paragraph itself (clean break vs. overlapping break) was already correct in revision 2 and is preserved unchanged.
2. **No-timing overstatement.** Revision 2's "What this doesn't cover" section stated "Cheiro's readings above don't specify when in life a break occurs or guarantee any particular result," which characterizes Cheiro's own historical system as containing no timing, no event, and no outcome — a historical negative claim the controlling evidence does not support (the evidence file states only that this task excludes Cheiro's timing/date system, not that no such system exists). Revision 3 restates this as a scope boundary on the article ("Cheiro's timing/date system is outside this article's scope... not a claim that his historical reading contained no timing, no event, or no outcome of its own") plus a separately labeled Palmistry Path editorial-policy statement ("Palmistry Path does not use this configuration to predict or guarantee any particular result"). The same correction was applied to the matching "Common mistakes" entry.

### Controlling evidence boundary

Unchanged from revision 2, per `.ai-ops/evidence/PP-RELAY-048-cheiro-fate-line-break.md`:
...
| Break = discontinuity between segments; clean break = gap along the line's course; overlapping break = new segment begins before the earlier ends | Direct observation, not attributed to any source, corrected in this revision to remove the universal-gap assertion while keeping the two-arrangement distinction consistent with item 2 above |
| Absence vs. break vs. fork observation vocabulary | Direct observation, not attributed to any source, consistent with `no-fate-line-meaning.md` and `forked-fate-line-meaning.md`'s existing observation sections |
| "This article doesn't teach timing" / "Palmistry Path does not use this configuration to predict or guarantee an outcome" | Palmistry Path editorial policy per `AGENTS.md`, explicitly labeled as a scope boundary and as site policy — not a claim about what Cheiro's own historical system did or didn't contain |

No statement in the article goes beyond this table. Every "What this doesn't cover" item (no career/life-change generalization, no smooth/planned/managed/gradual/beneficial transition, no shifted-restart-means-new-field or multiple-life-chapters claim, no gap-size scale, no mount/line combination) names a claim explicitly in order to decline it, not to assert it.

### Preflight disposition

1. **Prevalence/consensus language.** `npm run audit:claim-risk -- src/content/blog/beginner/broken-fate-line-meaning.md` returns 0 findings against the revision-3 text. A manual grep for `most|many|often|commonly|typically|generally|usual|rare|frequently` finds only the "Frequently asked questions" heading, which is not a claim.
2. **Scientific/historical assertions.** Limited to the three items in the claim-level evidence map above, all traceable to `.ai-ops/evidence/PP-RELAY-048-cheiro-fate-line-break.md` and attributed to Cheiro by name. The corrected timing statement is now scoped to what this article teaches, not to a historical claim about what Cheiro's own system contained.
...
4. **No vague anonymous authority.** No "modern palmists," "some writers," "traditional readers," "experts," or equivalent phrase appears; every historical claim is attributed to Cheiro by name.
5. **Claim-type separation.** The "Observation, history, and site policy — kept separate" section, plus the corrected break-definition and timing paragraphs and common-mistakes entries, keep direct observation, Cheiro's named historical interpretation, and Palmistry Path's own non-deterministic editorial policy in distinct, explicitly labeled statements.
...
7. **Safety boundaries.** No medical, legal, financial, deterministic relationship, danger, illness, death, or predictive-science claim appears. The corrected timing/outcome statement in "What this doesn't cover," the common mistakes section, and two FAQ answers keep the exclusion framed as this article's scope and Palmistry Path's policy, not as a historical claim about Cheiro's system.

**Claim-risk heuristic disposition:** `npm run audit:claim-risk -- src/content/blog/beginner/broken-fate-line-meaning.md` — 0 findings against the revision-3 text.

**Duplication/cannibalization check:** Compared against `no-fate-line-meaning.md` (absence, not interruption — the article opens by naming and linking this distinction and does not restate no-fate-line-meaning's own "broken or fragmented" paragraph, which contains unverified legacy wording — see follow-up note below), `forked-fate-line-meaning.md` (division into two continuing strands, a different formation, cross-linked both directions), and the main `fate-line.md` pillar (broader treatment; the new article narrows specifically into the break/overlap evidence boundary and is linked from the pillar's existing "Breaks and interruptions" section).

**Reciprocal navigation:** Added one sentence to the end of `fate-line.md`'s existing "Breaks and interruptions" paragraph, linking to the new article for the specific verified overlapping-break evidence — the same single addition made in revision 2, unchanged in substance.

**Verification:** `npm run audit:claim-risk -- src/content/blog/beginner/broken-fate-line-meaning.md` (0 findings). `npm run build`. `npm run content-audit`. `npm run audit:all`. `git diff --check`.
...
## Pass 16 — Fate Line pillar and absence companion: legacy break/fragmentation cleanup, revision 2: removed an unsupported negative-outcome claim (Relay PP-RELAY-049, revision 2, 2026-08-25)

Revision 1 (PR #88, superseded) corrected the two pre-existing legacy defects recorded in the follow-up note above, both outside PP-RELAY-048's scope: `fate-line.md`'s "Breaks and interruptions" section (plus its FAQ and "Common myths" echoes) and `no-fate-line-meaning.md`'s "Broken or fragmented fate line" paragraph. Director review found one residual defect in revision 1's own new prose, isolated to a single sentence: the "Common myths" entry stated that Cheiro's verified break reading was "not with failure or ruin, and not with any fixed outcome," and that the overlapping-break reading was "not a verdict on the life as a whole." The controlling evidence establishes the positive historical associations only; it does not establish what Cheiro's reading did or did not mean about failure, ruin, or any fixed outcome. Revision 2 preserves the sound revision-1 diff in full and corrects only that sentence. Uses the same controlling evidence as revision 1 and Pass 15, `.ai-ops/evidence/PP-RELAY-048-cheiro-fate-line-break.md`, and no new evidence.

### Controlling evidence boundary

Unchanged from revision 1 and Pass 15:
- A fate line broken or made up of small pieces throughout its course — Cheiro historically associated this with troubles/breaks in the career and a lack of settled or continuous success.
- An overlapping break (one segment begins before the other ends) — Cheiro historically associated this narrower configuration with a complete change in surroundings and position.
- A conditional addition: where the new segment is itself good and straight, Cheiro associated it with advancement in position from the point where the new segment begins.
...
The "Common myths and oversimplifications" entry for a broken fate line in `fate-line.md` previously read: "Cheiro historically associated a broken or piecemeal fate line with career troubles and a lack of settled or continuous success — not with failure or ruin, and not with any fixed outcome. For an overlapping break specifically, his historical reading was narrower still: a complete change in surroundings and position, not a verdict on the life as a whole." It now reads: "Cheiro historically associated a broken or piecemeal fate line with career troubles and a lack of settled or continuous success. For an overlapping break specifically, his historical reading was narrower still: a complete change in surroundings and position. Palmistry Path does not use either reading to predict or guarantee a fixed outcome."

The correction removes every characterization of what Cheiro's reading excluded (failure, ruin, "any fixed outcome," "a verdict on the life as a whole") without introducing a synonym or replacement claim about outcomes Cheiro did not supply. The retained reassurance — that the site does not use either reading to predict or guarantee a fixed outcome — is stated strictly as Palmistry Path's own editorial policy, separate from and following the accurate named Cheiro association, matching the wording and structure already used in the same article's FAQ answer ("Palmistry Path doesn't treat either form as predicting a fixed outcome"), which Director review did not flag.

### Claim-level evidence map

| Article statement | Evidence basis |
|---|---|
| `fate-line.md` "Breaks and interruptions": clean break = gap along the line's course; overlapping break = new segment begins before the earlier one ends | Direct observation, not attributed to any source, consistent with `broken-fate-line-meaning.md`'s established vocabulary |
| `fate-line.md` "Breaks and interruptions", FAQ, and "Common myths" entry: general piecemeal/broken line = troubles/breaks, lack of settled or continuous success, attributed to Cheiro | Verified evidence boundary, item 1 above |
| Same three locations: overlapping break = complete change in surroundings and position, attributed to Cheiro | Verified evidence boundary, item 2 above |
| "Breaks and interruptions" conditional advancement reading, kept with the "good and straight" condition attached | Verified evidence boundary, item 3 above |
| FAQ and "Common myths" entry: "Palmistry Path doesn't treat either form as predicting a fixed outcome" / "Palmistry Path does not use either reading to predict or guarantee a fixed outcome" | Palmistry Path editorial policy per `AGENTS.md`, explicitly labeled as site policy in both locations, not a claim about what Cheiro's own historical reading did or did not mean |
| `no-fate-line-meaning.md` "Broken or fragmented fate line": observational description only (sections/gaps, distinct from absence), with a link to `broken-fate-line-meaning.md` for the verified interpretation | Direct observation; no interpretive claim retained in this paragraph |

No statement in either corrected passage goes beyond this table. The removed wording — "not with failure or ruin," "not with any fixed outcome," and "not a verdict on the life as a whole," alongside revision 1's earlier removal of "significant change in direction," "planned or smoother transition," "sharper shift," "significant changes of direction," "multiple chapters rather than one thread," and the unattributed "classical texts"/"contemporary practice" framing — is named here only to record its removal, not retained anywhere in the corrected prose.

### Preflight disposition

1. **Prevalence/consensus language.** `npm run audit:claim-risk -- src/content/blog/beginner/fate-line.md src/content/blog/beginner/no-fate-line-meaning.md` returns 20 findings, the same as revision 1, all pre-existing `PREVALENCE_OR_CONSENSUS` matches ("most," "many," "often") in passages this task's scope does not touch (fate line starting positions, cross-tradition framing, hand-comparison guidance, and similar). None fall within the break/fragmentation passages, FAQ answer, or Common myths entry corrected by this task. Not dispositioned further as out of scope for this narrow task.
2. **Scientific/historical assertions.** Limited to the three items in the claim-level evidence map above, all traceable to `.ai-ops/evidence/PP-RELAY-048-cheiro-fate-line-break.md` and attributed to Cheiro by name. The revision-2 correction narrows this further: no historical assertion in the corrected passages now characterizes what Cheiro's reading did or did not include beyond those three items.
3. **No invented combination readings.** Neither corrected passage combines the verified break/overlap readings with any mount, other major line, endpoint, branch, fork, depth, or island.
4. **No vague anonymous authority.** The corrected prose does not introduce any anonymous-authority phrase; every retained historical claim is attributed to Cheiro by name.
5. **Claim-type separation.** The "Breaks and interruptions" subsection, FAQ answer, and Common myths entry keep the observational clean-break/overlapping-break distinction, Cheiro's named historical interpretation, and the separately labeled Palmistry Path policy statement (no fixed-outcome prediction) distinguishable throughout, including in the corrected Common myths sentence.
6. **Quotation fidelity.** No text is presented as a verbatim Cheiro quotation anywhere in the corrected passages; both evidence-based readings are paraphrased, consistent with the controlling evidence note.
7. **Safety boundaries.** No medical, legal, financial, deterministic relationship, danger, illness, death, or predictive-science claim appears. The FAQ's and Common myths entry's fixed-outcome exclusions are now both framed exclusively as Palmistry Path's own policy, not as a claim about what Cheiro's historical reading did or did not mean.

**Claim-risk heuristic disposition:** `npm run audit:claim-risk -- src/content/blog/beginner/fate-line.md src/content/blog/beginner/no-fate-line-meaning.md` — 20 findings, all pre-existing and outside the corrected passages (see item 1 above).

**Negative-outcome-claim search:** Searched the cumulative changed prose and documentation for `not with failure`, `not with any fixed outcome`, `failure`, `ruin`, `fixed outcome`. The only remaining occurrences in the changed files are: `fate-line.md`'s pre-existing, out-of-scope "Ending at the head line" passage ("The framing of ruin is not how current palmistry reads this configuration" — a different configuration, not part of this task's break/fragmentation scope); `no-fate-line-meaning.md`'s pre-existing, out-of-scope absence-related passages ("Assuming career failure or professional drift," "none treats absence as a verdict of failure" — about an absent line, not a broken one, also outside this task's scope); and the two now-policy-labeled "fixed outcome" statements in the FAQ and Common myths entry described above. No unsupported negative historical claim about Cheiro's break reading remains.

**Consistency check:** Compared the corrected `fate-line.md` and `no-fate-line-meaning.md` passages against `broken-fate-line-meaning.md`'s established clean-break/overlapping-break vocabulary and its own claim-level evidence map (Pass 15); the corrected passages use the same observational vocabulary and the same two attributed Cheiro readings, and introduce no contradiction.
...
## Pass 17 — Intuition Line Meaning in Palmistry, revision 4: hunch-accuracy myth and "do and don't claim" wording corrections — `blog/beginner/intuition-line-palmistry.md` (Relay PP-RELAY-050, revision 4, 2026-08-25)

Scope: Relay PP-RELAY-050 revision 4 (`SOURCE_SENSITIVE`), publishing
`src/content/blog/beginner/intuition-line-palmistry.md`. Revision 1 (PR #91,
closed without merging) built a six-file package bounded by
`.ai-ops/evidence/PP-RELAY-050-cheiro-benham-intuition-line.md`, but Director
review found four bounded source-integrity/correctness defects. Revision 2
(PR #92, closed without merging) corrected those four defects — removing the
unqualified "line of clairvoyance" mention outside the FAQ, removing the
unsupported "writing decades apart" chronology, replacing the "easiest to
confuse" superlative with neutral "A nearby look-alike is" wording, and
separating the myths/FAQ historical association from Palmistry Path's own
policy statement instead of characterizing Cheiro or Benham as denying
psychic/clairvoyant claims — and passed CI plus independent Director reruns
of the claim-risk scan, build, content audit, full audit suite, and
`git diff --check`. Director review of revision 2 found three residual
source-boundary defects in Benham's historical-association paragraph and two
of the "Common myths" entries. Revision 3 (PR #93, closed without merging)
corrected those three defects — bounding Benham's paragraph to the approved
intuitive-impressions/estimating-people association, removing the unsupported
"a claim about degree that Cheiro doesn't" inference from source silence, and
removing the psychic-power myth's "not a claim about paranormal ability"
negative characterization and the Mercury-line myth's "easy error" likelihood
claim — and passed CI plus independent Director reruns of the claim-risk
scan, build, content audit, full audit suite, and `git diff --check`.
Director cumulative review of revision 3 found one remaining Benham
source-integrity contradiction and two directly equivalent summary phrases.
Revision 3 was never merged, so revision 4 was drafted directly from
revision 3's file content, correcting only the three flagged locations below
plus this log/changelog/result bookkeeping, and preserving revision 3's
sound structure, evidence map, navigation, and bookkeeping approach
otherwise unchanged.

### What changed from revision 3

| Revision 3 statement | Problem | Revision 4 disposition |
|---|---|---|
| "Cheiro's and Benham's historical associations are with a sensitive or impressionable temperament, not with the accuracy of any specific dream, warning, or hunch." (hunch-accuracy myth) | Negatively characterized what Cheiro's and Benham's historical accounts did not associate with accuracy — an inference the controlling evidence does not support; the evidence explicitly warns that Benham made period accuracy claims that must not become Palmistry Path guidance, so the article cannot assert the authors' historical readings were unconcerned with accuracy. | Rewritten to state only the approved historical association (a sensitive, impressionable temperament and intuitive impressions), followed by Palmistry Path's own, separately stated policy that it does not use the line to validate or guarantee that an impression, hunch, dream, or warning is accurate or reliable — no claim about what the authors' historical reading was or wasn't about. |
| "...what their historical readings do and don't claim." (meta description) | Broad, unbounded claim about what the authors' historical readings did not assert — the same negative-characterization pattern flagged in the hunch myth, repeated in the page's own metadata. | Replaced with a plain 148-character description naming the location, the Mercury-line distinction, and the named historical associations, with no claim about what the authors did or didn't claim. |
| "...what their historical readings do and don't claim." (introduction, parallel to the description) | Same negative-characterization pattern repeated in the article's opening paragraph. | Replaced with neutral wording: the article covers the two authors' named historical associations and how Palmistry Path separates those associations from its own editorial policy — no claim about the historical readings' scope or omissions. |

No other article passage, the frontmatter, the article structure, navigation,
the reciprocal link, or backlog count/status changed from revision 3.

### What changed from revision 2

| Revision 2 statement | Problem | Revision 3 disposition |
|---|---|---|
| "...with sensitivity in estimating other people — reading character and situations through impression rather than deliberate analysis." | Added "situations," an implied cognitive mechanism ("through impression"), and a contrast with "deliberate analysis" that the controlling evidence does not establish — Benham's approved association is intuitive impressions and sensitivity in estimating people, not a description of how that estimation cognitively works or what it excludes. | Removed the added clause. The sentence now stops at the approved association: intuitive impressions and sensitivity in estimating other people. |
| "Benham also makes a claim about degree that Cheiro doesn't..." | Asserted that Cheiro omitted or declined a degree claim — an inference from Cheiro's source silence that the controlling evidence does not support (the evidence records what Cheiro's texts say, not an exhaustive account of what they don't). | Rewritten to "Benham also gives a deep-versus-broken degree reading specific to this line," stating only Benham's own approved historical scale without any comparison to what Cheiro did or didn't claim. |
| "...a named historical interpretation, not a claim about paranormal ability." (psychic-power myth) | Negatively characterized what Cheiro's and Benham's historical reading *was not* — an editorializing gloss on the authors' own intent that the controlling evidence does not establish; the evidence supports only their positive historical association, not a claim about what they meant to exclude. | Removed. The entry now states the approved sensitive/impressionable association plainly, followed immediately by Palmistry Path's own separately stated policy that the site does not treat the marking as evidence of psychic or clairvoyant power — no claim about the authors' intent. |
| "Mistaking one for the other is an easy error, exactly because Cheiro says the two can run together on the same hand." (Mercury-line look-alike myth) | Unsupported ease/likelihood claim — the evidence establishes that the markings can run alongside one another and remain distinct, not that confusing them is easy, common, or likely. | Removed. The entry now closes by restating the approved facts only: the markings sit in the same territory, can run alongside one another per Cheiro, remain distinct, and are told apart by Benham's shape test (Mercury line straighter, Intuition line curved). |

No other article passage, the frontmatter, the article structure, navigation,
the reciprocal link, or backlog count/status changed from revision 2.

### Controlling evidence boundary

- Cheiro and Benham both name the feature **Line of Intuition**.
- Both describe a curved or near-semicircular line on the outer/percussion
  side between the Luna and Mercury regions, with slightly different
  directional wording (Cheiro: face of Mercury to Luna, or confined to Luna;
  Benham: rising on Luna, curving across the Plain of Mars, ending on/near
  Mercury).
- Both distinguish it from the Mercury/Health line (Hepatica); Cheiro notes
  the markings can run through or alongside one another while remaining
  distinct; Benham's distinguishing test is shape (Mercury line straighter,
  Intuition line curved).
- Cheiro historically associates the line with sensitivity to
  impressions/surroundings, presentiments, vivid dreams, inspiration, and
  intuitive expression.
- Benham historically associates a well-marked line with intuitive
  impressions and sensitivity in estimating people, and states a
  deep-versus-broken degree reading specific to this line.
- The evidence does **not** establish "Line of Clairvoyance," "Psychic
  Line," or any equivalent phrase as a verified alternate historical name.

### Claim-level evidence map

| Article statement | Evidence basis |
|---|---|
| Feature named the Line of Intuition | Controlling evidence boundary, item 1 |
| Location: curved/near-semicircular path, outer/percussion side, between Mount of Mercury and Mount of Luna, with Cheiro's and Benham's differing directional wording both stated | Controlling evidence boundary, item 2 |
| Distinction from the Mercury/Health line: Benham's straighter-vs-curved test; Cheiro's markings can run through/alongside one another while remaining distinct | Controlling evidence boundary, item 3 |
| Cheiro's historical associations (sensitivity to impressions/surroundings, presentiments, vivid dreams, inspiration, intuitive expression in speech/writing) | Controlling evidence boundary, item 4, attributed to Cheiro by name |
| Benham's historical associations (intuitive impressions, sensitivity in estimating other people) and his named deep-versus-broken degree statement, stated as his own claim with no comparison to Cheiro | Controlling evidence boundary, item 5, attributed to Benham by name; revision-3 correction removed the added "situations"/cognitive-mechanism wording and the "that Cheiro doesn't" comparison |
| FAQ: "Line of Clairvoyance" is not a verified alternate name in the sources checked; other unreviewed palmistry writing may use it differently | Evidence file's "Important source disagreement / naming limit" section; the single permitted body location for this phrase |
| "Palmistry Path's approach to this line" and the corrected myths/FAQ entries: historical interpretation vs. site policy, no validation of psychic/clairvoyant ability, no accuracy claim for any impression, stated separately from and without characterizing what Cheiro or Benham's reading was or wasn't | Palmistry Path editorial policy per `AGENTS.md`, stated as this site's own position; revision-3 correction removed the "not a claim about paranormal ability" negative characterization of the authors' intent; revision-4 correction removed the hunch-accuracy myth's "not with the accuracy of any specific dream, warning, or hunch" negative characterization, keeping only the approved historical association plus the separately stated policy |
| Introduction and meta description: the article covers the two authors' named historical associations and how Palmistry Path separates those associations from its own editorial policy | Palmistry Path editorial policy per `AGENTS.md`; revision-4 correction removed both instances of the unbounded "what their historical readings do and don't claim" wording, which asserted a scope for the authors' historical readings the evidence does not establish |
| Mercury-line look-alike myth: markings occupy nearby territory, can run alongside one another per Cheiro, remain distinct, distinguished by Benham's shape test — no claim about ease or likelihood of confusing them | Controlling evidence boundary, item 3; revision-3 correction removed the unsupported "easy error" claim |
| Myths/FAQ: absence not addressed by either author so not interpreted; the Intuition and Mercury lines are distinct markings | Direct negations bounded by "What this evidence does not support" in the evidence file; no new claim introduced |

No statement in the article goes beyond this table. No cross-tradition,
Gettings, Fincham, West, prevalence/percentage, medical, source-silence
inference, or unauthorized combination claim was introduced.

### Preflight disposition

1. **Prevalence/consensus language.** None remains. The removed "easy error"
   claim was the only likelihood/frequency-adjacent statement in revision 2,
   and it is gone.
2. **Scientific/historical assertions.** Every historical claim traces to the
   claim-level evidence map above and is attributed to Cheiro or Benham by
   name; no chronology, study, source-silence inference, or scientific claim
   appears. Revision 4 removed the hunch-accuracy myth's unsupported
   assertion that the authors' historical accounts were "not with the
   accuracy" of any impression — the controlling evidence warns that
   Benham's period accuracy claims must not become Palmistry Path guidance,
   so the article no longer characterizes what their historical reading did
   or didn't cover on that point.
3. **No invented combination readings.** The only cross-feature statement is
   the authorized Mercury-line distinction (location and shape only); no
   mount, other major/minor line, hand/finger shape, star, cross, island,
   branch, fork, or endpoint combination appears.
4. **No vague anonymous authority.** Searched
   `Gettings|Fincham|West|Indian|Chinese|modern palmists|contemporary practitioners|some writers|experts`
   — zero matches in the article.
5. **Claim-type separation.** "Where to find it" (observation), "What it's
   traditionally associated with" (Cheiro's paragraph, then Benham's
   paragraph, each named), and "Palmistry Path's approach to this line"
   (site policy) are distinct sections; the myths and FAQ keep the same
   separation, and no entry characterizes what Cheiro's or Benham's reading
   was or wasn't beyond their positive named association.
6. **Quotation fidelity.** No quotation marks are used against Cheiro's or
   Benham's wording anywhere in the article; both authors' readings are
   paraphrased throughout, so no verbatim verification was required.
7. **Safety boundaries.** No medical, legal, financial, deterministic
   relationship, danger, or predictive-science claim appears; the
   psychic/clairvoyant-proof and impression-accuracy claims are declined as
   Palmistry Path's own policy, stated separately from the named historical
   associations and without characterizing the authors' intent.

**Restricted-term search (preflight item 4):**
`clairvoyance|clairvoyant|psychic|medium|supernatural|prophetic|empath|rare|seldom|most|many|often|commonly|typically|generally|usually|accurate|reliable|predict`
against the article returns matches limited to: the FAQ's permitted
"Line of Clairvoyance" synonym question and psychic/clairvoyant question
(required and permitted by the task packet), the corrected myths/FAQ
entries' named historical association plus separately stated Palmistry Path
policy (no proof/accuracy/reliability claim), "Palmistry Path's approach to
this line"'s policy statements, and one regex substring false positive
(`unusually` contains "usually"). No unqualified alias, prevalence, proof,
or prediction claim remains, and no restricted term characterizes what
Cheiro or Benham did or didn't claim. Revision 4's rewritten hunch-accuracy
myth still matches on "accurate"/"reliable" only inside Palmistry Path's own
policy statement, not as a claim about the authors' historical reading.

**Unsupported-attribution search (preflight item 5):**
`Gettings|Fincham|West|Indian|Chinese|modern palmists|contemporary practitioners|some writers|experts`
— zero matches.

**Combination-reading search (preflight item 6):** No mount, other line,
hand/finger shape, star, cross, island, branch, fork, or endpoint combination
appears; the only cross-feature material is the authorized Mercury-line
location/shape distinction. Benham's named "Plain of Mars" location wording
(a location the line's course passes through, per the controlling evidence)
is retained; it is not an interpretive combination with the Mount of Mars.
Benham's deep-versus-broken degree statement is retained, named, historical,
and non-empirical, with no comparison to Cheiro.

**Consistency check (preflight item 9):** Compared against
`minor-lines-overview.md` (does not currently name the Intuition Line as one
of its four covered minor lines — no contradiction), `mount-of-luna.md`
(attributes its broader imaginative/intuitive territory directly to Cheiro
and Benham by name in its own body text — no contradiction),
`mercury-line.md` (describes a diagonal line toward Mercury, consistent with
the article's straighter-vs-curved distinction — no contradiction), and
`m-line-palmistry.md` (its existing "small curved line on the percussion
side of the hand" description of the intuition line matches the article's
location description — no contradiction). This pass's wording changes only
narrow existing claims and add no new factual assertion, so the comparison
result is unchanged from revision 3.

**Source-note check (preflight item 10):** The article's source note lists
only Cheiro (*Palmistry for All*, 1916; *Cheiro's Language of the Hand*) and
Benham (*The Laws of Scientific Hand Reading*, 1900) — the two sources
actually used. The backlog's original unverified Gettings/Fincham naming
note is not presented as evidence anywhere in the article, and the backlog
entry itself records the correction (`docs/editorial-backlog.md` item 20).

**Rejected-claim search (preflight item 11):** Searched the cumulative
article and documentation for
`writing decades apart|easiest to confuse|Neither Cheiro nor Benham|Nothing in either author's account|other classical writers|covers how this line|line of clairvoyance`.
In the article, "Line of Clairvoyance" occurs only inside the single
permitted FAQ question and its answer; no other phrase in this list appears.
`docs/editorial-backlog.md` item 20's published notes and this pass's
revision-1-through-2 summary above retain these phrases only as past-tense
task history explaining what revision 1 contained and revision 2 corrected
— not repeated as approved prose.

**Rejected-claim search (preflight item 12):** Searched the cumulative
article and documentation for
`situations through impression|rather than deliberate analysis|Cheiro doesn't|not a claim about paranormal ability|easy error`.
None remain in the article. This log's "What changed from revision 2" table
above and the corresponding CHANGELOG entry retain them only as past-tense
task history explaining their removal.

**Rejected-claim search (preflight item 13):** Searched the cumulative
article and documentation for
`not with the accuracy|readings do and don't claim|readings do and don't claim`.
None remain in the article: the hunch-accuracy myth, the meta description,
and the introduction were each rewritten to remove them. This log's "What
changed from revision 3" table above and the corresponding CHANGELOG entry
retain them only as past-tense task history explaining their removal. The
revised description is confirmed 148 characters, inside the required
140–160 range.

**Reciprocal navigation:** Unchanged from revision 1, revision 2, and
revision 3. Added
one reciprocal link in `m-line-palmistry.md`'s existing "The M signals strong
intuition" myth entry, turning its existing unlinked mention of "the
intuition line" into a link to the new article — a minimal edit to
already-present text, not a rewrite. No reciprocal link was added into
`minor-lines-overview.md` (it does not currently name the Intuition Line as
one of its four covered minor lines; adding one would mean introducing a
fifth minor-line section, a broader rewrite outside this task's scope) or
`mount-of-luna.md` (it discusses the mount's general intuitive quality but
does not name this specific line, and inserting a new named-line reference
there risked implying the mount's broad intuitive association is the same
thing as the specific Line of Intuition feature, which the evidence does not
establish). The new article links out once each to the Mercury line, Mount
of Luna, and the minor lines overview.

**Verification:** `npm run audit:claim-risk -- src/content/blog/beginner/intuition-line-palmistry.md` (1 finding, the disclosed myth-heading pattern, unchanged from revision 3). `npm run build` (117 pages). `npm run content-audit` (72 blog posts, 25 lessons valid). `npm run audit:all` (links incl. orphan check, images, schema, indexability, accessibility all pass). `git diff --check` (clean).
