...
## Legacy Head Line pillar claim-boundary cleanup — `blog/beginner/head-line.md` (Relay PP-RELAY-042, revision 8, 2026-08-23)

Revisions 1–6 of PP-RELAY-042 were never merged to `main`; revision 7 correctly discovered this and returned `HUMAN_REQUIRED`. Revision 8 repairs the baseline error and reconstructs the bounded cleanup in one pass against the actual pre-cleanup article on current `main`, which still carried every issue revisions 1–6 had attempted to fix, plus the two passages PP-RELAY-041 (evidence note `.ai-ops/evidence/PP-RELAY-041-benham-head-line-direction.md`) had already flagged as noted-but-out-of-scope for this file specifically (unsupported monotonic slope/degree wording and Cheiro clinical-insanity framing).

The claim-risk audit against the untouched `main` article returned 26 findings (25 `PREVALENCE_OR_CONSENSUS`, 1 `PRECISION_OR_DEGREE_EXTRAPOLATION`), matching revision 7's prior count exactly. A full manual re-read then found five further categories of risk the word-list heuristic does not catch: monotonic degree-scaling phrased with words outside its list ("the longer... the longer", "the degree of slope matters... more pronounced... dominant", "intensifies this... correlates with"), vague anonymous-authority attribution ("the classical writers", "other writers confirm", "classical texts", "contemporary writers", "one contemporary practitioner"), an unsupported naming-history claim ("which is how the name spread"), two invented combination readings synthesizing independently-supported single-line meanings into a new joint interpretation, and exhaustive-tradition IQ/vocation claims ("The tradition says nothing about cognitive ability") that should be Palmistry Path editorial policy rather than a claim about the whole tradition, per the task packet.

**Findings changed (unsupported, narrowed, or removed):**

| Passage (pre-edit) | Claim as published | What the approved evidence supports | Action |
|---|---|---|---|
| Intro | "the heart line gets the most anxious attention, the life line gets the most persistent misreading" | No repository evidence supports a claim about reader/audience behavior toward the other lines; not a palmistry interpretation claim at all. | Removed; intro now reads "the head line is easy to overlook next to the heart line and life line," keeping the verified, named Cheiro "most important" quotation intact. |
| Where to find it | "It typically begins on the radial side..." / "Its path and endpoint are among its most significant features." | No repository evidence establishes a physical-prevalence rate for head-line origin across hands in general, or ranks path/endpoint as the single most significant feature. | "typically" removed (direct instructional description); ranking claim narrowed to "traditionally significant features to observe," matching the PP-RELAY-040 precedent for the same failure mode. |
| Where to find it | "The head line usually starts at or near the same origin as the life line" | No repository evidence establishes a prevalence rate for joined-vs-separate origins (same finding class as PP-RELAY-040 line 16/28). | "usually" removed; sentence restated as a direct description of the relationship being read. |
| What it's traditionally associated with | "This is the consistent Western formulation" | No repository evidence establishes cross-source consistency for Benham's specific framing beyond Benham himself. | Narrowed to "In this framing" (i.e., Benham's), removing the Western-tradition-wide consensus claim. |
| Starting point | "This is the variation Western palmistry discusses most specifically at the head line" | No repository evidence ranks this variable's discussion-frequency against the head line's other variables (same failure mode as PP-RELAY-040 line 30/36/42). | Ranking claim removed; reworded to "carries close traditional attention," unranked. |
| Starting point — joined | "The classical writers note that the longer the two lines remain joined before separating, the longer this period of influence tends to extend." | `PRECISION_OR_DEGREE_EXTRAPOLATION`-class finding, unflagged by the word list. No repository evidence establishes a graduated, monotonic scale between joined-duration and degree of influence; "the classical writers" is also an unattributed vague-authority framing. | Sentence removed (matches the PP-RELAY-038/040 precedent of dropping unsupported degree-scaling rather than replacing it). |
| Starting point — moderate | "The moderate separation is generally the most positively read position in the Western tradition" | No repository evidence supports a Western-tradition-wide ranking; only Cheiro's own quoted "excellent mark" reading is verified. | Narrowed to "In Cheiro's reading, this is initiative without recklessness" — attributed to Cheiro, not the tradition at large. |
| Starting point — wide | "...and other writers confirm this general reading. The wider the gap, the more strongly this quality is associated." | `PRECISION_OR_DEGREE_EXTRAPOLATION` heuristic match plus an unattributed "other writers" claim; no repository evidence for either the corroboration or a graduated gap-to-impulsiveness scale. | Both sentences removed; the verified Cheiro "excessive impetuosity" quotation stands alone. |
| Starting point — overlapping | "has been associated in classical texts with..." / "Contemporary writers have reframed this as..." | Vague plural-group attribution ("classical texts", "contemporary writers") with no identifiable approved source, duplicating the specific, already-verified Cheiro attribution that follows. | Vague lead sentence removed; the softened framing is now attributed directly to Palmistry Path's own editorial choice rather than an anonymous group, consistent with the site's standing §5.2 policy of stating the site's own framing as its own. |
| Curvature — gentle slope | "The degree of slope matters: a gentle downward curve adds... a more pronounced slope indicates that imaginative and intuitive tendencies are dominant..." | Evidence note item 3: the creative/artistic stronger-slope rule "is not established as a monotonic degree rule." | Sentence removed; the single qualitative association (gentle slope → imaginative/intuitive tendencies) is retained without the degree escalation. |
| Curvature — steep slope | "...intensifies this: the tradition associates it with a highly developed inner life, strong artistic or creative tendencies... Cheiro associated a sharply sloping head line with extreme morbid imagination and a tendency to withdraw from ordinary life... The underlying observation... correlates with exceptional imaginative capacity..." | Evidence note items 3–5: no monotonic degree rule is established; Cheiro's clinical/insanity framing is "historical and unsafe for modern instructional use... unnecessary for this article"; the "correlates with" phrasing invents an empirical-correlation claim no repository evidence supports. | Fully rewritten to Evidence note item 3's narrow, safe form: Benham's historical examples of writers/speakers/creative professions among deeper-sloping lines, explicitly offered as examples rather than a degree rule. Clinical language and the invented correlation claim removed entirely. |
| Writer's fork | "is among the most discussed special features in Western palmistry. It is commonly called..." | No repository evidence ranks this feature's discussion-frequency, or establishes a usage-prevalence rate for the "writer's fork" name. | Ranking/prevalence claims removed; now "is a notable feature. In the popular Western tradition it is known as..." (attribution pattern matching the log's existing "writer's fork" term-reassignment fix, line 426 above). |
| Writer's fork | "Writers and lawyers are both named because each profession draws on this dual capacity..." | Asserts an unverified naming-history/chronology reason for the dual name; no repository evidence establishes why the two professions were named. | Causal naming-history claim removed; sentence now states only the trait itself (the capacity to see from more than one angle) without explaining the name's origin. |
| Writer's fork | "as one contemporary practitioner puts it plainly, 'many people have forks but no special writing talent...' ... It appears among writers with notable frequency, which is how the name spread; it does not appear exclusively among them, and its absence indicates nothing about creative potential." | Vague anonymous-authority quotation with no identifiable approved source; unsupported naming-history claim; population association/non-association claim with writers/creative ability — exactly what the task packet's evidence boundary prohibits. | Invented quotation and prevalence/naming-history/population claims all removed. Replaced with the repository's own verified Cheiro quotation (log line 426 above: "gives more of what is called a dual mentality and less power of concentration on any one subject") and an explicit Palmistry Path editorial-policy statement ("does not treat the fork as proof of writing or creative ability, and does not treat its absence as evidence against them either"), matching the task packet's suggested policy framing. |
| Chains/breaks/islands/branches | "Upward branches... are generally read positively" | No repository evidence establishes tradition-wide consensus for this reading. | "generally" removed; reads positively without the consensus qualifier. |
| Double head line | "is uncommon... Classical texts associated it with... more common forks and branches" | No repository evidence establishes a population-frequency rate for double head lines vs. forks/branches (same failure mode as PP-RELAY-040 line 48/72); "Classical texts" is an unattributed vague-authority lead-in. | "uncommon"/"more common" prevalence pair removed; vague lead-in replaced with the site's sanctioned "traditionally associated with" phrasing. |
| Head line in context | "the interaction between them is often where the most revealing observations occur. A strongly expressive heart line combined with a steeply sloping head line suggests a temperament where emotion and imagination are both active and likely in productive tension..." plus the following "brief illustration" paragraph (moderate separation + gentle slope + fork + long heart line → "independence, imaginative breadth... warmth and emotional engagement... relational rather than merely interior") | Both passages combine independently-supported single-line meanings into new, specific synthesized readings not themselves established in approved evidence — exactly what the controlling-evidence boundary's "no invented combination readings" rule prohibits, alongside an unsupported "often... most revealing" prevalence claim. | Both replaced with a single general methodological sentence (reading head and heart line together, rather than in isolation, gives a fuller picture than either alone) that asserts no specific new combined interpretation. The "brief illustration" paragraph is removed rather than narrowed, since a narrowed version would still assert the same invented combination. |
| FAQ — location | "typically at or near the same origin" | Same unattributed-prevalence finding as the "Where to find it" passage above. | "typically" removed. |
| FAQ / myths — IQ | "The tradition says nothing about cognitive ability." / "The tradition says nothing about IQ or cognitive ability." | Per the task packet: IQ/cognitive-ability safety wording must be Palmistry Path editorial policy, not an exhaustive claim about what "the tradition" says. | Both reworded to "Palmistry Path does not read head-line length as a measure of cognitive ability" / "...IQ or cognitive ability" — stated as the site's own editorial policy. |
| FAQ — writer's fork | "It appears in many hands, not exclusively in writers'." | Population-prevalence and non-association claim with writers, unsupported. | Reworded to the same Palmistry Path editorial-policy framing used in the main writer's-fork section. |
| FAQ — steep slope | "The steeper the slope, the more dominant those tendencies are..." | `PRECISION_OR_DEGREE_EXTRAPOLATION` heuristic match; same unsupported monotonic rule as the main curvature section. | Degree-scaling sentence removed; answer ends at the single qualitative association. |
| Myths — fork | "It appears frequently in writers' hands; it also appears in many other hands, and its absence does not prevent writing." | Population-prevalence and association/non-association claim with writers, unsupported. | Reworded to the same Palmistry Path editorial-policy framing ("does not treat it as proof of writing or creative ability, or its absence as evidence against it"). |
| Myths — mental illness | "The Victorian tendency to read particular head line features as clinical conditions is not how the contemporary tradition works" | Asserts, without repository evidence, what "the contemporary tradition" broadly does or does not do — the exact carried-forward/not-carried-forward claim the task packet flags. | Reworded to state the historical clinical framing existed ("some Victorian-era writers") and that Palmistry Path does not use it here — the site's own editorial choice, not a claim about the state of the wider tradition. |

**Findings reviewed and left unchanged (compliant or already-verified):**

| Passage | Wording | Disposition |
|---|---|---|
| Intro (×2) | "the most important line" / "the most important sign that can be found in the hand" | Verified verbatim Cheiro quotation (*Palmistry for All*, 1916) and its direct paraphrase reference; a named, verified attribution, not an anonymous prevalence claim. Retained, matching the PP-RELAY-040 line 20/22/116 precedent above. |
| Length | "considers many angles" | False positive: describes the character of an individual mind's own range of thinking, not a population-frequency claim about hands. Retained. |
| Depth and clarity | "great brain workers usually have thin, fine, clean looking lines" | Verified Cheiro quotation; "usually" is Cheiro's own word inside quotation marks, not new site-authored prevalence language. Not re-verified against the primary text in this bounded task (out of scope per packet item 8); left as the log's existing "kept verified" status for this file records it. Retained. |
| Myths — straight line | "Creativity takes many forms" | False positive: a general statement about the concept of creativity itself, not a palmistry population/prevalence claim. Retained. |
| Length | Benham quote "the kind to be estimated by the character and color of the line" | Already corrected to the verified wording per this log's Pass 5 finding (line 409 above, "Benham writes 'the **kind** to be **estimated** by…'"). Confirmed still correct on `main`; unchanged. |

**Direction/path boundary:** confirmed straight → Benham/practical mentality and sloping-toward-Luna → Benham/imagination are the only direction claims retained, matching `.ai-ops/evidence/PP-RELAY-041-benham-head-line-direction.md` exactly. No fixed angle/zone/average cutoff or empirical-correlation claim remains.

**Writer's-fork vocation/talent caution:** now stated twice as explicit Palmistry Path editorial policy (main section and FAQ) plus once in the myths section, never as a population claim. **IQ/cognitive-ability caution:** now stated as explicit Palmistry Path editorial policy (FAQ and myths), not an exhaustive tradition-wide claim. **No risky phrase was replaced by an equivalent unsupported synonym** — every action above either removes wording outright or replaces it with a named attribution (Benham, Cheiro) or an explicit Palmistry Path editorial-policy statement.

**Claim-type separation:** direct observation (where/how to look), named historical interpretation (Cheiro/Benham, attributed), and Palmistry Path's own editorial policy (IQ, vocation/talent, mental-illness framing) are distinguishable throughout the revised article.

**Quotation fidelity:** no existing quotation mark was altered; the one new quotation added (Cheiro's "dual mentality" line) is copied verbatim from this log's own Pass 5 verification (line 426 above), not newly sourced.

**Verification:** `npm run audit:claim-risk -- src/content/blog/beginner/head-line.md` returns 26 findings before editing (matching revision 7's count) and 5 after — all five dispositioned above as verified quotations or harmless non-palmistry false positives, none newly introduced. `npm run build` (110 pages), `npm run content-audit` (65 blog posts, 25 lessons valid), `npm run audit:all` (links — including the no-orphan check — images, schema, indexability, accessibility all pass), and `git diff --check` (clean) all pass. Diff is bounded to the article, this log entry, `docs/CHANGELOG.md`, and the revision-8 result artifact; no other article, the canonical lesson, or claim-risk tooling was touched.
