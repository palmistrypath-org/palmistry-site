...
## Legacy Head Line pillar — `blog/beginner/head-line.md` source-integrity cleanup, revision 4 (Relay PP-RELAY-042, revision 4, 2026-08-23)

Revision 3 (PR #68) was never merged to `main`. Cumulative Director review of that PR found two residual anonymous-authority/consensus overstatements inside the same authorized cleanup category: the writer's-fork naming sentence generalized "neither Cheiro nor Benham uses [the label]" — a claim about two named authors — into "rather than one from the classical texts," a chronology/classical-text-wide claim no approved evidence establishes; and the intelligence/IQ FAQ and myth wording ("The tradition says nothing about cognitive ability" / "...about IQ or cognitive ability") asserted an exhaustive tradition-wide consensus that no approved source verifies. Because none of revisions 1–3's edits ever reached `main`, this revision reconstructs all sound revision-1 through revision-3 corrections (as captured on the unmerged revision-3 branch) against current `main` and applies the required revision-4 remediation on top, in one pass.

**Revision-1 through revision-3 corrections reconstructed and re-confirmed present during this revision:** all corrections recorded in this log's revision-1, revision-2, and revision-3 entries above (the unattributed consensus/ranking removals, the degree-scaling removals, the Cheiro clinical-framing replacement with Benham's writer/poet illustrations, the unverifiable "one contemporary practitioner" quotation removal, and the four revision-3 population/comparative-prevalence corrections in the writer's-fork and double-head-line passages) were re-verified against the current article text and against `.ai-ops/evidence/PP-RELAY-041-benham-head-line-direction.md` for the direction/path boundary. None was reopened or altered by this revision.

**Revision-4 corrections (the two Director-identified residual passages):**

| Location | Passage before this revision | Why it is an anonymous-authority/consensus overstatement | Action |
|---|---|---|---|
| Writer's fork naming sentence | "The label is a later popular-palmistry term rather than one from the classical texts — neither Cheiro nor Benham uses it — known as the 'writer's fork,' and sometimes the 'lawyer's fork.'" | The only verified evidence is that two named authors, Cheiro and Benham, do not use the label. Generalizing that to "the classical texts" as a category (and to a "later" chronology relative to them) asserts something about every classical text, which two authors' non-use does not establish. No approved repository evidence supports the broader chronology/classical-text claim. | Narrowed to the actually verified fact only: "Neither Cheiro nor Benham uses the label; it is known instead as the 'writer's fork,' and sometimes the 'lawyer's fork.'" No chronology or classical-text-wide claim remains. |
| Intelligence/IQ FAQ + myth wording | "The tradition says nothing about cognitive ability." / "The tradition says nothing about IQ or cognitive ability." | Asserts an exhaustive claim about everything the entire palmistry tradition has ever said, which no approved source verifies — a tradition-wide consensus overstatement of the same category the task authorizes cleaning up. | Restated as Palmistry Path's own bounded interpretation rather than a claim about the tradition: "This article does not treat length as a measure of intelligence, IQ, or cognitive ability." (FAQ) and "This article does not treat length as a measure of IQ or cognitive ability." (myth section). The intended safety point — length is not a measure of cognitive ability — is preserved without an exhaustive-consensus claim. |

Neither revision-4 edit introduces a new palmistry meaning, an invented chronology, or a replacement synonym carrying the same unsupported consensus/authority claim.

**Re-read of the two revision-4 passages for directly equivalent anonymous-authority/consensus wording:** the writer's-fork naming sentence's remaining "traditional interpretation" phrasing (next sentence) follows this log's established, previously-accepted convention for citing the tradition generally without inventing a named or anonymous authority; it is not a new consensus claim. The IQ/cognitive-ability passages contain no further prevalence, consensus, or anonymous-authority wording after the correction. No other passage in these two locations required action.

**No unsupported phrase was replaced by an equivalent unsupported synonym:** the corrected wording states only what the approved evidence verifies (two named authors' non-use) or what is Palmistry Path's own stated editorial position (the IQ/cognitive-ability framing), with no substitute chronology, population, or consensus term introduced.

**Direction/path evidence boundary:** unchanged and re-verified against `.ai-ops/evidence/PP-RELAY-041-benham-head-line-direction.md`; this revision made no change to the direction/path passages.

**Independent re-scan for every remaining heuristic finding:** `npm run audit:claim-risk -- src/content/blog/beginner/head-line.md` returns 10 findings, matching the revision-3 count exactly (lines 9, 11, 17, 45, 49, 89, 114 — rhetorical scene-setting, verified direct quotations, and Palmistry Path's own editorial statements, none a population/consensus/anonymous-authority claim). Neither revision-4 edit location appears among the findings. No finding was dismissed without being checked against the article's sourced content and this task's cleanup categories.

**No invented combination readings:** the "brief illustration" paragraph is unchanged from revision 3 and remains explicitly framed as an illustration of the site's cross-line reading methodology, combining only individually-established readings from elsewhere in the article.

**Vague anonymous authority:** none remains in the two revision-4 categories after this revision's edits. "Traditionally associated with" phrasing elsewhere in the article follows this log's established, accepted convention.

**Quotation fidelity:** no quotation marks were added, removed, or altered by this revision. The pre-existing Cheiro/Benham quotations are unchanged.

**Safety boundaries:** no medical, legal, financial, deterministic-relationship, or predictive-science claim was introduced or remains in the article.

**Material source concerns outside the two revision-4 categories:** none newly identified. The Pass 1 (2026-08-10) quotation-verification gap remains logged and unexpanded, unchanged from the revision-3 entry.

**Verification:** `npm run audit:claim-risk -- src/content/blog/beginner/head-line.md` — 10 findings, unchanged from revision 3 and independently re-confirmed compliant above. `npm run build` (110 pages), `npm run content-audit` (65 blog posts, 25 lessons valid), `npm run audit:all` (links — including the no-orphan check — images, schema, indexability, accessibility all pass), and `git diff --check` (clean) all pass. Diff (against current `main`) is bounded to the reconstructed revision-1 through revision-3 corrections plus the two revision-4 corrected passages, this log entry, `docs/CHANGELOG.md`, and the revision-4 result artifact. No Broken Head Line article, canonical lesson, claim-risk tooling, or unrelated article was touched.

---

## Legacy Head Line pillar — `blog/beginner/head-line.md` source-integrity cleanup, revision 5 (Relay PP-RELAY-042, revision 5, 2026-08-23)

Revision 4 (PR #69) was never merged to `main`. Cumulative Director review of that PR found the writer's-fork subsection still carried two directly equivalent anonymous-authority/naming-history claims within the same authorized cleanup category, even after the revision-4 chronology narrowing: the fork was still described as "a specific feature in Western palmistry" (an unattributed categorization/prevalence-adjacent claim), and — after the verified fact that neither Cheiro nor Benham uses the label — the sentence still asserted "it is known instead as the 'writer's fork,' and sometimes the 'lawyer's fork,'" an anonymous-authority claim about who uses each label with no approved evidence, and an unverified "lawyer's fork" synonym found nowhere else in the repository (not even in the sibling `forked-head-line-meaning.md` article, which discusses only "writer's fork"). Because none of revisions 1–4's edits ever reached `main`, this revision reconstructs all sound revision-1 through revision-4 corrections (as captured on the unmerged revision-4 branch) against current `main` and applies the required revision-5 remediation on top, in one pass.

**Revision-1 through revision-4 corrections reconstructed and re-confirmed present during this revision:** all corrections recorded in this log's revision-1 through revision-4 entries above (the unattributed consensus/ranking removals, the degree-scaling removals, the Cheiro clinical-framing replacement with Benham's writer/poet illustrations, the unverifiable "one contemporary practitioner" quotation removal, the revision-3 population/comparative-prevalence corrections, and the revision-4 chronology and IQ-consensus narrowing) were re-verified against the current article text and against `.ai-ops/evidence/PP-RELAY-041-benham-head-line-direction.md` for the direction/path boundary. None was reopened or altered by this revision.

**Revision-5 correction (the writer's-fork naming/anonymous-authority passage):**

| Location | Passage before this revision | Why it is an anonymous-authority/naming-history overstatement | Action |
|---|---|---|---|
| Writer's-fork opening sentence + naming clause | "A fork at the end of the head line — where it divides near its terminus, with one branch continuing relatively straight and another sloping toward Luna — is a specific feature in Western palmistry. Neither Cheiro nor Benham uses the label; it is known instead as the 'writer's fork,' and sometimes the 'lawyer's fork.'" | "Is a specific feature in Western palmistry" is an unattributed categorization claim implying broader tradition-wide recognition than any approved evidence establishes. "It is known instead as..." is an anonymous-authority claim (known by whom, how widely) about both labels. "Lawyer's fork" specifically has no approved-evidence support anywhere in the repository — not in this article's own prior source review, not in the sibling `forked-head-line-meaning.md` article (which discusses only "writer's fork" and never mentions "lawyer's fork"), and not in `docs/source-verification-log.md`'s existing writer's-fork disposition (line ~426, above). Cheiro/Benham's non-use of a label does not establish who does use a different label, per this task's controlling boundary. | Narrowed to the verified fact plus this article's own stated naming convention, dropping the unsupported "lawyer's fork" synonym entirely: "A fork at the end of the head line — where it divides near its terminus, with one branch continuing relatively straight and another sloping toward Luna — is referred to in this article as the 'writer's fork.' Neither Cheiro nor Benham uses that label." No claim about who else uses either label, how common the configuration is, or when either name arose. |
| Following sentence's naming-history clause | "...Writers and lawyers are both named because each profession draws on this dual capacity — the ability to inhabit more than one perspective, to hold the practical and the imaginative in dialogue." | This sentence's premise (that a "lawyer's fork" label exists and needs explaining) depended entirely on the unsupported "lawyer's fork" claim just removed; retaining it would have left an equivalent unsupported naming-history claim by explaining why an unverified second label supposedly exists. | Removed. The preceding sentence's Cheiro-grounded "dual mentality" interpretation (already verified per this log's earlier writer's-fork entry, line ~426, quoting Cheiro's "dual mentality... less power of concentration on any one subject") is retained unchanged, since it describes the interpretation itself rather than a naming-history claim. |

Neither correction introduces a new palmistry meaning, a replacement prevalence/consensus/chronology claim, or an unsupported synonym in place of the removed wording — "lawyer's fork" is dropped rather than re-justified, per the task's explicit instruction not to substitute a synonym.

**Re-read of the complete writer's-fork subsection for directly equivalent anonymous-authority, prevalence, consensus, naming-history, or chronology language:** the remaining two paragraphs — the "dual mentality" interpretive sentence and the closing paragraph stating the fork does not track writing talent in either direction (a revision-2 correction, unchanged) — contain no further naming-history, prevalence, consensus, or anonymous-authority wording. No other equivalent phrasing was found in this subsection.

**No unsupported phrase was replaced by an equivalent unsupported synonym:** "is referred to in this article as" attributes the label to this article's own editorial convention (verifiably true — the term is used throughout this article and the site) rather than asserting an external "known as" fact; no chronology, population, or consensus term was introduced in place of "lawyer's fork" or "a specific feature in Western palmistry."

**Direction/path evidence boundary:** unchanged and re-verified against `.ai-ops/evidence/PP-RELAY-041-benham-head-line-direction.md`; this revision made no change to the direction/path passages.

**Independent re-scan for every remaining heuristic finding:** `npm run audit:claim-risk -- src/content/blog/beginner/head-line.md` returns 10 findings, matching the revision-4 count exactly (lines 9, 11, 17, 45, 49, 89, 114 — rhetorical scene-setting, verified direct quotations, and Palmistry Path's own editorial statements, none a population/consensus/anonymous-authority claim). Neither revision-5 edit location appears among the findings.

**No invented combination readings:** the "brief illustration" paragraph is unchanged from revision 4 and remains explicitly framed as an illustration of the site's cross-line reading methodology, combining only individually-established readings from elsewhere in the article.

**Vague anonymous authority:** none remains in the writer's-fork subsection after this revision's edits. "Traditionally associated with" phrasing elsewhere in the article follows this log's established, accepted convention and was not reopened.

**Quotation fidelity:** no quotation marks were added, removed, or altered by this revision.

**Safety boundaries:** no medical, legal, financial, deterministic-relationship, or predictive-science claim was introduced or remains in the article.

**Material source concerns outside the revision-5 category:** none newly identified. The Pass 1 (2026-08-10) quotation-verification gap remains logged and unexpanded, unchanged from the revision-4 entry.

**Verification:** `npm run audit:claim-risk -- src/content/blog/beginner/head-line.md` — 10 findings, unchanged from revision 4 and independently re-confirmed compliant above. `npm run build` (110 pages), `npm run content-audit` (65 blog posts, 25 lessons valid), `npm run audit:all` (links — including the no-orphan check — images, schema, indexability, accessibility all pass), and `git diff --check` (clean) all pass. Diff (against current `main`) is bounded to the reconstructed revision-1 through revision-4 corrections plus the writer's-fork subsection corrected in this revision, this log entry, `docs/CHANGELOG.md`, and the revision-5 result artifact. No canonical lesson, claim-risk tooling, or unrelated article was touched.

---

## Legacy Head Line pillar — `blog/beginner/head-line.md` source-integrity cleanup, revision 6 (Relay PP-RELAY-042, revision 6, 2026-08-23)

Revision 5 (PR #70) was never merged to `main`. Cumulative Director review found the writer's-fork subsection's remaining talent/creative-potential sentence — "The fork does not track writing talent in either direction... neither its presence nor its absence indicates anything about creative potential" — asserted a population-level empirical non-association between the fork and writing/creative talent that no approved evidence establishes. The approved evidence supports Cheiro's historical "dual mentality" interpretation of the fork; it does not establish, and no approved source was found to establish, what population data do or do not show about a relationship (or lack of one) between the fork and writing/creative talent. Because none of revisions 1–5's edits ever reached `main`, this revision reconstructs all sound revision-1 through revision-5 corrections (verified identical to the unmerged revision-5 branch) against current `main` and applies the required revision-6 remediation on top, in one pass.

**Revision-1 through revision-5 corrections reconstructed and re-confirmed present during this revision:** the working-tree copy of `src/content/blog/beginner/head-line.md` on the unmerged `claude/relay-PP-RELAY-042-head-line-source-cleanup-r5` branch was diffed directly against current `main` and confirmed to differ only by the corrections recorded in this log's revision-1 through revision-5 entries above (the unattributed consensus/ranking removals, the degree-scaling removals, the Cheiro clinical-framing replacement with Benham's writer/poet illustrations, the revision-3 population/comparative-prevalence corrections, the revision-4 chronology and IQ-consensus narrowing, and the revision-5 writer's-fork naming/anonymous-authority narrowing). That reconstructed text was used as the starting point for this revision, and the direction/path passages were re-verified against `.ai-ops/evidence/PP-RELAY-041-benham-head-line-direction.md`. None was reopened or altered by this revision.

**Revision-6 correction (the writer's-fork talent/creative-potential passage):**

| Location | Passage before this revision | Why it is an unsupported empirical/population claim | Action |
|---|---|---|---|
| Writer's-fork closing paragraph | "The fork does not track writing talent in either direction. It is an indicator of mental flexibility — the capacity to operate in multiple registers — not a certificate of vocation, and neither its presence nor its absence indicates anything about creative potential." | "Does not track writing talent in either direction" and "neither its presence nor its absence indicates anything about creative potential" both assert what population data show (or fail to show) about the fork's relationship to writing/creative talent. The approved evidence supports Cheiro's historical dual-mentality interpretation of the fork; it does not establish a modern empirical population-level relationship or non-relationship with writing/creative talent. No approved source in the repository verifies this negative empirical claim. | Reframed as Palmistry Path's own stated editorial caution rather than an evidence claim: "Palmistry Path does not treat the fork, in its presence or its absence, as proof of writing or creative ability." The safety point — that the fork is not proof of writing/creative talent — is preserved without asserting what population data do or do not show. |

The correction does not introduce a new palmistry meaning, an invented population claim, or a replacement synonym carrying the same unsupported empirical-association wording.

**Re-read of the complete writer's-fork subsection for directly equivalent empirical, population, prevalence, consensus, naming-history, chronology, or anonymous-authority wording:** the remaining sentences — the "referred to in this article as" naming convention, "Neither Cheiro nor Benham uses that label," and "The name reflects the traditional interpretation..." (the Cheiro-grounded dual-mentality sentence) — contain no further empirical, population, prevalence, consensus, naming-history, chronology, or anonymous-authority wording. No other equivalent phrasing was found in this subsection.

**No unsupported phrase was replaced by an equivalent unsupported synonym:** "Palmistry Path does not treat... as proof of" attributes the caution to this article's own stated editorial policy rather than to what evidence, tradition, or population data show; no population, frequency, or consensus term was introduced in place of the removed wording.

**Direction/path evidence boundary:** unchanged and re-verified against `.ai-ops/evidence/PP-RELAY-041-benham-head-line-direction.md`; this revision made no change to the direction/path passages.

**IQ/cognitive-ability passages:** re-inspected (FAQ and myths section) and confirmed unchanged from revision 4 — both remain framed as Palmistry Path's own editorial interpretation ("This article does not treat length as a measure of intelligence, IQ, or cognitive ability" / "...IQ or cognitive ability"), not a tradition-wide consensus claim.

**Independent re-scan for every remaining heuristic finding:** `npm run audit:claim-risk -- src/content/blog/beginner/head-line.md` returns 10 findings, matching the revision-5 count exactly (lines 9, 11, 17, 45, 49, 89, 114 — rhetorical scene-setting, verified direct quotations, and Palmistry Path's own editorial statements, none a population/consensus/anonymous-authority/empirical-association claim). The revision-6 edit location does not appear among the findings.

**No invented combination readings:** the "brief illustration" paragraph is unchanged from revision 5 and remains explicitly framed as an illustration of the site's cross-line reading methodology, combining only individually-established readings from elsewhere in the article.

**Vague anonymous authority:** none remains in the writer's-fork subsection after this revision's edit. "Traditionally associated with" phrasing elsewhere in the article follows this log's established, accepted convention and was not reopened.

**Quotation fidelity:** no quotation marks were added, removed, or altered by this revision.

**Safety boundaries:** no medical, legal, financial, deterministic-relationship, or predictive-science claim was introduced or remains in the article.

**Material source concerns outside the revision-6 category:** none newly identified. The Pass 1 (2026-08-10) quotation-verification gap remains logged and unexpanded, unchanged from the revision-5 entry.

**Verification:** `npm run audit:claim-risk -- src/content/blog/beginner/head-line.md` — 10 findings, unchanged from revision 5 and independently re-confirmed compliant above. `npm run build`, `npm run content-audit`, `npm run audit:all`, and `git diff --check` all pass (see the durable result artifact for exact counts). Diff (against current `main`) is bounded to the reconstructed revision-1 through revision-5 corrections plus the writer's-fork talent/creative-potential sentence corrected in this revision, this log entry, `docs/CHANGELOG.md`, and the revision-6 result artifact. No canonical lesson, claim-risk tooling, or unrelated article was touched.
