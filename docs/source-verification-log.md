...
## Pass 17 — Intuition Line Meaning in Palmistry, new article within a Director-verified Cheiro/Benham evidence boundary, revision 2: source-integrity corrections to unmerged revision 1 (Relay PP-RELAY-050, revision 2, 2026-08-25)
...
Revision 1 (PR #91, closed without merging) built a six-file package that passed the claim-risk scan, build, content audit, full audit suite, CI, and `git diff --check`, but Director review found four bounded source-integrity/correctness defects in its prose. Revision 1 did not pass final Director source review; this pass records revision 2's correction of those four defects, not an endorsement of revision 1's prose as previously written.

### Revision 1 defects corrected in revision 2

1. The opening paragraph named "line of clairvoyance" outside the FAQ. Removed; the opening now states only the verified Cheiro/Benham naming and location facts, with the unverified-synonym question left for the FAQ.
2. The opening paragraph claimed Cheiro and Benham were "writing decades apart" — an unsupported chronology inference the controlling evidence does not establish. Removed, with no replacement chronology claim; the named works and their verified dates remain.
3. The location section claimed the Mercury line is "the line easiest to confuse it with" — an unsupported superlative about which identification error is most likely. Replaced with "A nearby look-alike is," a neutral observation-first framing that makes no claim about frequency or likelihood.
4. Two "Common myths" entries characterized Cheiro and Benham as denying psychic/clairvoyant claims ("Neither Cheiro nor Benham makes that claim"; "Nothing in either author's account ... supports treating an impression as reliable"), which attributes a modern evidentiary restraint to authors whose period vocabulary the controlling evidence notes can itself include terms like "clairvoyance," "psychic," and "medium" inside their historical interpretations. Both entries, and the FAQ's parallel psychic/clairvoyant question, now state only the verified named historical association (a sensitive, impressionable temperament) and separately state Palmistry Path's own editorial policy (the site does not treat the line as evidence of paranormal ability and does not use it to validate or guarantee an impression's accuracy or reliability) — without characterizing what Cheiro or Benham did or didn't claim about psychic power itself.

The "What comes next" section's unsupported "other classical writers" attribution and its inaccurate description of `minor-lines-overview.md` as covering this line were also corrected in the same pass (`minor-lines-overview.md` currently names four other minor lines, not the Line of Intuition; the link is now described as general context rather than specific coverage). The Mount of Luna link is unchanged, since `mount-of-luna.md` independently attributes its broader imaginative/intuitive territory to Cheiro and Benham by name (verified directly against that file).
...
| FAQ: "Line of Clairvoyance" is not a verified alternate name in the sources checked; other unreviewed palmistry writing may use it differently | Evidence file's "Important source disagreement / naming limit" section; the qualifier avoids inferring the phrase is false or absent across all traditions; the single permitted body location for this phrase per the revision-2 corrections |
| "Palmistry Path's approach to this line" and the corrected myths/FAQ entries: historical interpretation vs. site policy, no validation of psychic/clairvoyant ability, no accuracy claim for any impression, stated separately from and without characterizing what Cheiro or Benham did or didn't claim | Palmistry Path editorial policy per `AGENTS.md`, stated as this site's own position, not attributed to Cheiro or Benham |
| Myths/FAQ: presence does not prove psychic ability (stated as a named historical association plus separate site policy, not a denial attributed to the authors); absence not addressed by either author so not interpreted; the Intuition and Mercury lines are distinct markings | Direct negations bounded by "What this evidence does not support" in the evidence file; no new claim introduced |
...
1. **Prevalence/consensus language.** `npm run audit:claim-risk -- src/content/blog/beginner/intuition-line-palmistry.md` returns 1 finding: the quoted myth heading `**"A Line of Intuition proves you're psychic."**`, immediately corrected in the same entry — the same disclosed pattern already published in other myth sections across the site; left in place.
2. **Scientific/historical assertions.** Every historical claim traces to the claim-level evidence map above and is attributed to Cheiro or Benham by name; no chronology, study, or scientific claim appears (the revision-1 "writing decades apart" chronology claim is removed).
...
4. **No vague anonymous authority.** Searched for `Gettings|Fincham|West|Indian|Chinese|modern palmists|contemporary practitioners|some writers|experts` — zero matches in the article. The revision-1 "other classical writers" attribution in "What comes next" is removed.
5. **Claim-type separation.** "Where to find it" (observation), "What it's traditionally associated with" (Cheiro's paragraph, then Benham's paragraph, each named), and "Palmistry Path's approach to this line" (site policy) are distinct sections; the corrected myths and FAQ keep the same separation, now without attributing a psychic/clairvoyant-claim denial to either author.
...
7. **Safety boundaries.** No medical, legal, financial, deterministic relationship, danger, or predictive-science claim appears; the psychic/clairvoyant-proof and impression-accuracy claims are declined as Palmistry Path's own policy, stated separately from the named historical associations.

**Restricted-term search (preflight item 4):** `clairvoyance|clairvoyant|psychic|medium|supernatural|prophetic|empath|rare|seldom|most|many|often|commonly|typically|generally|usually|accurate|reliable|predict` against the article returns matches limited to: the opening's location description (no restricted term), the FAQ's declined "Line of Clairvoyance" synonym question and psychic/clairvoyant question (required and permitted by the task packet), the corrected myths/FAQ entries' named historical association plus separately stated Palmistry Path policy (no proof/accuracy/reliability claim), "Palmistry Path's approach to this line"'s policy statements, and two regex substring false positives (`unusually` contains "usually"; "consistent across" and "the same general territory... across" are not the marking "cross"). No unqualified alias, prevalence, proof, or prediction claim remains, and no restricted term is used to characterize what Cheiro or Benham did or didn't claim.
...
**Combination-reading search (preflight item 6):** No mount, other line, hand/finger shape, star, cross, island, branch, fork, or endpoint combination appears; the only cross-feature material is the authorized Mercury-line location/shape distinction. Benham's named "Plain of Mars" location wording (a location the line's course passes through, per the controlling evidence) is retained; it is not an interpretive combination with the Mount of Mars.

**Consistency check (preflight item 9):** Compared against `minor-lines-overview.md` (does not currently name the Intuition Line as one of its four covered minor lines — confirmed directly against that file; no contradiction, and the "What comes next" link is now described as general context rather than specific coverage), `mount-of-luna.md` (attributes its broader imaginative/intuitive territory directly to Cheiro and Benham by name in its own body text — confirmed directly against that file, supporting the corrected "What comes next" sentence; no contradiction), `mercury-line.md` (describes a diagonal line toward Mercury, consistent with the article's straighter-vs-curved distinction; no contradiction), and `m-line-palmistry.md` (its existing "small curved line on the percussion side of the hand" description of the intuition line matches the article's location description exactly; no contradiction).

**Source-note check (preflight item 10):** The article's source note lists only Cheiro (*Palmistry for All*, 1916; *Cheiro's Language of the Hand*) and Benham (*The Laws of Scientific Hand Reading*, 1900) — the two sources actually used. The backlog's original unverified Gettings/Fincham naming note is not presented as evidence anywhere in the article, and the backlog entry itself records the correction (see `docs/editorial-backlog.md` item 20).

**Rejected-claim search (preflight item 11):** Searched the cumulative article and documentation for `writing decades apart|easiest to confuse|Neither Cheiro nor Benham|Nothing in either author's account|other classical writers|covers how this line|line of clairvoyance`. In the article, "Line of Clairvoyance" occurs only inside the single permitted FAQ question and its answer. In `docs/editorial-backlog.md`'s item 20 entry, the phrases occur only inside the past-tense "Published notes" explaining what revision 1 contained and revision 2 corrected — task-history documentation, not a repeated claim. An unrelated "neither Cheiro nor Benham" occurrence in this log's Pass 2 (2026-08-10) describes a different, already-corrected life-line defect from an unrelated task and is out of scope for this pass. No other occurrence of any of these phrases remains anywhere in the cumulative changed prose or documentation.

**Reciprocal navigation:** Unchanged from revision 1. Added one reciprocal link in `m-line-palmistry.md`'s existing "The M signals strong intuition" myth entry, turning its existing unlinked mention of "the intuition line" into a link to the new article — a minimal edit to already-present text, not a rewrite. No reciprocal link was added into `minor-lines-overview.md` (it does not currently name the Intuition Line as one of its four covered minor lines; adding one would mean introducing a fifth minor-line section, a broader rewrite outside this task's scope) or `mount-of-luna.md` (it discusses the mount's general intuitive quality but does not name this specific line, and inserting a new named-line reference there risked implying the mount's broad intuitive association is the same thing as the specific Line of Intuition feature, which the evidence does not establish). The new article links out once each to the Mercury line, Mount of Luna, and the minor lines overview.

**Verification:** `npm run audit:claim-risk -- src/content/blog/beginner/intuition-line-palmistry.md` (1 finding, the disclosed myth-heading pattern above). `npm run build` (117 pages). `npm run content-audit` (72 blog posts, 25 lessons valid). `npm run audit:all` (links incl. orphan check, images, schema, indexability, accessibility all pass). `git diff --check` (clean).
